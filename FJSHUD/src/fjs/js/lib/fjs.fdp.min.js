...
function(a,f,g){d.getVersions(f,b,function(){g()})},function(){d.transport.send({type:"synchronize",data:{versions:b}})}):Error("You must set feeds names array")};fjs.fdp.SyncManager.prototype.saveVersions=function(a,b,d){this.db&&(void 0!==d&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.fdp.TabsSynchronizer).isMaster))&&this.db.insertOne("versions",{feedSource:a+"_"+b,feedName:a,source:b,version:d})};fjs.fdp.SyncManager.prototype.getVersions=function(a,b,d){var e=
[],f=this;this.versions[a]?setTimeout(function(){d(b[a]=f.versions[a])},0):this.db?this.db.selectByIndex("versions",{feedName:a},function(a){e.push(a.source+"@"+a.version)},function(){f.versions[a]=b[a]=e.join(",");d(b[a])}):setTimeout(function(){d(f.versions[a]=b[a]="")},0)};fjs.fdp.SyncManager.prototype.lazySyncProcessItems=function(a,c,d){for(var e=[],f=0;f<a.length;f++){var g=a[f],h=g.xef001type||b.eventTypes.ENTRY_CHANGE,j=g.xpid=g.xpid?g.xpid:d?d+"_"+g.xef001id:g.xef001id;delete g.xef001type;
g.source=d;g={eventType:h,feed:c,xpid:j,entry:g};if(this.db)switch(h){case b.eventTypes.ENTRY_CHANGE:e.push(g.entry);break;case b.eventTypes.ENTRY_DELETION:(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.fdp.TabsSynchronizer).isMaster)&&this.db.deleteByKey(c,g.xpid,null);break;default:console.error("Incorrect item change type: "+h+" for Lazy sync")}this.fireEvent(c,g)}this.db&&(0<e.length&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.fdp.TabsSynchronizer).isMaster))&&
this.db.insertArray(c,e,null)};fjs.fdp.SyncManager.prototype.fullSyncProcessItems=function(a,c,d){for(var e=this,f=[],g=0;g<a.length;g++){var h=a[g],j=h.xef001type||b.eventTypes.ENTRY_CHANGE;h.source=d;if(j===b.eventTypes.ENTRY_CHANGE){var i=h.xpid=h.xpid?h.xpid:d?d+"_"+h.xef001id:h.xef001id;delete h.xef001type;j={eventType:j,feed:c,xpid:i,entry:h};f.push(h);this.fireEvent(c,j)}else console.error("Incorrect item change type: "+j+" for Full sync")}this.db&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||
...
case b.eventTypes.ENTRY_KEEP:f.push(i.entry.xpid);break;default:console.error("Incorrect item change type: "+k+" for Keep sync")}this.fireEvent(c,i)}this.db&&(0<e.length&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.fdp.TabsSynchronizer).isMaster))&&this.db.selectByIndex(c,{source:d},function(a){0>f.indexOf(a.xpid)&&0>g.indexOf(a.xpid)&&h.db.deleteByKey(c,a.xpid,null)},function(){h.db.insertArray(c,e,null)})};fjs.fdp.SyncManager.prototype.processFeedData=function(a,
...
this.processFeedData(c,this.syncs[a][c]);this.fireEvent(null,{eventType:b.eventTypes.SYNC_COMPLETE});this.syncs=[]}else console.log("!!!!Double onSync!!",this.syncs[0],a)};fjs.fdp.SyncManager.prototype.sendAction=function(a,b,d){d.action=b;this.transport.send({type:"action",data:{feedName:a,actionName:b,parameters:d}})};fjs.fdp.SyncManager.prototype.onClientSync=function(a,b){fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()&&(new fjs.fdp.TabsSynchronizer).isMaster?fjs.fdp.transport.LocalStorageTransport.masterSend("message",
{type:"sync",data:a}):fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()&&!b&&fjs.fdp.transport.LocalStorageTransport.masterSend("clientSync",a);if(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.fdp.TabsSynchronizer).isMaster)this.onSync(a)};fjs.fdp.SyncManager.prototype.getAuthTicket=function(){var a=this;this.state==b.states.READY&&this.db?this.db.clear(function(){a.fireEvent("authError",{type:"authError",message:"Auth ticket wrong or expired"})}):
...
function(a,b,d,e,f){0==e.length?f&&f(!1):(this.historyversions[a][b]=e,b={"s.limit":d,"sh.filter":b,"sh.versions":e.join("@")},this.transport.send({type:"loadNext",data:{feedName:a,data:b}}),f&&f(!0))};fjs.fdp.SyncManager.prototype.saveHistoryVersions=function(a,b,d,e){if(this.db&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.fdp.TabsSynchronizer).isMaster))if(this.db.insertOne("historyversions",{feedSourceFilter:a+"_"+b+"_filter",feedName:a,source:b,filter:d,version:e}),
b=this.historyversions[a])delete b[d],b||delete this.historyversions[a]};fjs.fdp.SyncManager.prototype.saveEmptyHistoryVersions=function(a,b){if(this.db&&(!fjs.fdp.TabsSynchronizer.useLocalStorageSyncronization()||(new fjs.fdp.TabsSynchronizer).isMaster))this.db.insertOne("historyversions",{feedName:a,filter:b,version:"-1"}),delete this.historyversions[a]};fjs.fdp.SyncManager.prototype.logout=function(){this.node=this.ticket=null;this.getAuthTicket();this.status=b.states.NOT_INITIALIZED};fjs.fdp.SyncManager.prototype.SFLogin=
