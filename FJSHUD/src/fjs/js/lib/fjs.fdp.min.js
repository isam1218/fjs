...
g.source=d;g={eventType:h,feed:c,xpid:i,entry:g};if(this.db)switch(h){case b.eventTypes.ENTRY_CHANGE:e.push(g.entry);break;case b.eventTypes.ENTRY_DELETION:(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.api.TabsSynchronizer).isMaster)&&this.db.deleteByKey(c,g.xpid,null);break;default:console.error("Incorrect item change type: "+h+" for Lazy sync")}this.fireEvent(c,g)}this.db&&(0<e.length&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.api.TabsSynchronizer).isMaster))&&
this.db.insertArray(c,e,null)};fjs.fdp.SyncManager.prototype.fullSyncProcessItems=function(a,c,d){for(var e=this,f=[],g=0;g<a.length;g++){var h=a[g],i=h.xef001type||b.eventTypes.ENTRY_CHANGE;h.source=d;if(i===b.eventTypes.ENTRY_CHANGE){var j=h.xpid=h.xpid?h.xpid:d?d+"_"+h.xef001id:h.xef001id;delete h.xef001type;i={eventType:i,feed:c,xpid:j,entry:h};f.push(h);this.fireEvent(c,i)}else console.error("Incorrect item change type: "+i+" for Full sync")}this.db&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||
...
case b.eventTypes.ENTRY_KEEP:f.push(j.entry.xpid);break;default:console.error("Incorrect item change type: "+k+" for Keep sync")}this.fireEvent(c,j)}this.db&&(0<e.length&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.api.TabsSynchronizer).isMaster))&&this.db.selectByIndex(c,{source:d},function(a){0>f.indexOf(a.xpid)&&0>g.indexOf(a.xpid)&&h.db.deleteByKey(c,a.xpid,null)},function(){h.db.insertArray(c,e,null)})};fjs.fdp.SyncManager.prototype.processFeedData=function(a,
