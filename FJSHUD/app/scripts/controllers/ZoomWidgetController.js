  ...
  settingsService.getSettings().then(function() {
   ...
   $http.get(fjs.CONFIG.SERVER.ppsServer +getURL('zoom/meetingList')+'&email='+$rootScope.meModel.email).success(function(response){
        ...
        for(var i = 0; i<=30;i++){
          ...
          $scope.meetingList[i].startHour = strt.tz($scope.meetingList[i].timezone).format('hh:mmA');
...
$scope.loading.meetingLoaded = true;
 $http.get(fjs.CONFIG.SERVER.ppsServer +getURL('zoom/meetingList')+'&email='+$rootScope.meModel.email).success(function(response){
        ...
        for(var i = 0; i<=30;i++){
          ...
          $scope.meetingList[i].startHour = jun.tz($scope.meetingList[i].timezone).format('hh:mmA');
