hudweb.directive('droppable',
				[
						...
						'$location',
						'$rootScope',
						function(httpService, conferenceService, settingsService, $parse, $timeout, $location, $rootScope) {
							var timeout;
									...
									$(element).droppable(
													{
														...
														drop : function(event, ui) {
															...
															// dock new item
															if ($(this).hasClass('InnerDock')
																&& ui.draggable[0].attributes.dockable) {
																// can't dock
																// yourself
																	...
																	var data = {
																		...
																		value : JSON.stringify(
																				{ "contextId" : "empty", "factoryId" : "Gadget" + type,
																				  "entityId" : obj.xpid,
																				  "config" : {
																						"x" : (ui.position.left - rect.left)
																								/ rect.width
																								* 100,
																						"y" : (ui.position.top - rect.top)
																								/ rect.height
																								* 100
																				 },
																	...
																	conferenceService.getConferences().then(
																					function(data) {
																						...
																						if (found) {
																							...
																							// add a brief timeout before adding the second user
																							// to the conference
																							timeout = setTimeout(
																									function() {
															...
															// normally
																	...
																	|| (scope.item && scope.item.recent_type == 'conference')
																	|| (scope.gadget && scope.gadget.name.indexOf('ConferenceRoom') != -1)) {
