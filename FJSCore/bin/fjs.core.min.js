...
(function(){namespace("fjs.utils");fjs.utils.Console=function(){};fjs.utils.Console.log=function(){console&&console.log.apply(console,arguments)};fjs.utils.Console.error=function(){console&&console.error.apply(console,arguments)};fjs.utils.Console.debug=function(){console&&(console.debug?console.debug.apply(console,arguments):console.log.apply(console,arguments))};fjs.utils.Console.warn=function(){console&&(console.warn?console.warn.apply(console,arguments):console.log.apply(console,arguments))};
fjs.utils.Console.info=function(){console&&(console.info?console.info.apply(console,arguments):console.log.apply(console,arguments))}})();
...
this.compatibility=!0;this.crdmnFrame?c.status=a.states.READY:(this.status=a.states.INITIALIZATION,this.crdmnFrame=document.createElement("iframe"),this.crdmnFrame.name=this.crdmnFrame.id="crdmnFrame",this.crdmnFrame.src=this.host,this.crdmnFrame.style.width=0,this.crdmnFrame.style.height=0,this.crdmnFrame.onload=function(){c.status=a.states.READY;for(var b=0;b<c.suspendRequestes.length;b++)c.send(null,null,null,null,null,c.suspendRequestes[b])},document.body.appendChild(this.crdmnFrame),window.addEventListener?
window.addEventListener("message",function(a){c.onResponse(a)},!1):window.attachEvent&&window.attachEvent("onmessage",function(a){c.onResponse(a)}))};fjs.ajax.IFrameAjax.states={NOT_INITIALIZED:-1,INITIALIZATION:0,READY:1};fjs.ajax.IFrameAjax.prototype.getParamData=function(a){var c=[],d;for(d in a)a.hasOwnProperty(d)&&c.push(d+"="+a[d]);return c.join("&")};fjs.ajax.IFrameAjax.prototype.send=function(b,c,d,f,g,e){this.compatibility?(e||(e=new fjs.ajax.IFrameRequest((new fjs.utils.Increment).get("IFrameAjaxRequest")),
e.method=b,e.data=this.getParamData(f),e.url=c,(b=d.Authorization)&&(b=b.replace("auth=","")),e.authTicket=b,e.node=d.node,this.listeners[e.id]=g),this.status!=a.states.READY)?this.suspendRequestes.push(e):(this.crdmnFrame.contentWindow.postMessage(JSON.stringify(e),"*"),this.requests[e.id]=e):(e||(e=new fjs.ajax.IFrameRequest((new fjs.utils.Increment).get("IFrameAjaxRequest")),e.method=b,e.postData=f,e.url=c,e.headers=d,this.listeners[e.id]=g),this.status!=a.states.READY)?this.suspendRequestes.push(e):
(this.crdmnFrame.contentWindow.postMessage(JSON.stringify({type:"send",data:e}),"*"),this.requests[e.id]=e);return e};fjs.ajax.IFrameAjax.prototype.abort=function(a){this.compatibility||this.crdmnFrame.contentWindow.postMessage(fjs.utils.JSON.stringify({type:"abort",data:a}),"*");a.aborted=!0;a.status=0;if(this.listeners[a.id])this.listeners[a.id](a,"",!1);delete this.listeners[a.id];delete this.requests[a.id]};fjs.ajax.IFrameAjax.prototype.onResponse=function(a){if(fjs.utils.JSON.check(a.data))if(a=
fjs.utils.JSON.parse(a.data),this.compatibility){if(a){var c=a.id,d=this.requests[c];if(d){d.status=a.status;d.responseText=a.response;var f=this.listeners[c];f&&(a.error?f(d,d.responseText,!1):f(d,d.responseText,!0),delete this.listeners[c])}delete this.requests[c]}}else if(a&&"IFrameAjaxRequest"==a.type){a=a.data;c=a.id;if(d=this.requests[c])if(d.status=a.status,d.responseText=a.responseText,f=this.listeners[c])a.error?f(d,d.responseText,!1):f(d,d.responseText,!0),delete this.listeners[c];delete this.requests[c]}}})();
