...
describe("LocalStorageTest", function() {
    ...
    it("check", function() {
            expect(true).toBe(fjs.db.LocalStorageDbProvider.check());
    });
    it("open", function(){
        if(fjs.db.LocalStorageDbProvider.check()) {
            var doneFlag, tablesCount = 0;
            runs(function(){
                var db = new fjs.db.LocalStorageDbProvider();
                db.open(dbConfig, function(){
                    doneFlag = true;
                });
            });
            waitsFor(function() {
        ...
        if(fjs.db.LocalStorageDbProvider.check()) {
            ...
            runs(function(){
                ...
                db.open(dbConfig, function(){
                    ...
                    db.insertArray("tTest2", items, function(){
                        db.selectByIndex("tTest2", {"field4": "test2", "field3":"11"}, function(item){
                                item1 = item;
                            },
                            function(items){
                                itemsCount = items.length;
