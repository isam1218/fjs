...
fjs.db.IndexedDBProvider.prototype.open=function(b,a,c){this.state=0;var d=this.indexedDB.open(b,a),e=this;d.onerror=function(c){Error("Error: can't open indexedDB ("+b+", "+a+")",c)};var f=function(a){Error("Database error: "+a.target.errorCode)};d.onsuccess=function(){e.db=d.result;e.db.onerror=f;e.state=1;c&&c(e.db)};d.onupgradeneeded=function(a){var b=e.db=a.target.result;for(a.target.transaction.onerror=b.onerror=f;0<b.objectStoreNames.length;)b.deleteObjectStore(b.objectStoreNames[0]);for(var c in e.tables)e.tables.hasOwnProperty(c)&&
...
fjs.db.LocalStorageDbProvider.prototype.open=function(b,a,c){var d,e=this;this.state=0;if(this.dbInfo=fjs.utils.JSON.parse(self.localStorage.getItem("DB_"+b)))if(a>this.dbInfo.version){for(var f=0;f<this.dbInfo.tables.length;f++)d=this.dbInfo.tables[f],self.localStorage.removeItem(d);this.dbInfo.tables=null;this.dbInfo.version=a;this.createTables();self.localStorage.setItem("DB_"+b,fjs.utils.JSON.stringify(this.dbInfo))}else for(b=0;b<this.dbInfo.tables.length;b++)d=this.dbInfo.tables[b],a=this.dbData[d]=
fjs.utils.JSON.parse(self.localStorage.getItem(d))||{},this.createIndexes(this.getRealTableName(d),a);else this.dbInfo={name:b,version:a},this.createTables(),self.localStorage.setItem("DB_"+b,fjs.utils.JSON.stringify(this.dbInfo));this.state=1;c&&setTimeout(function(){c(e)},0)};fjs.db.LocalStorageDbProvider.check=function(){return!!self.localStorage};fjs.db.LocalStorageDbProvider.prototype.createTable=function(){};
...
fjs.utils.JSON.stringify(a));localStorage.setItem("lsp_"+b,fjs.utils.JSON.stringify(a))}})();
...
fjs.fdp.SyncManager.prototype.onMasterChanged=function(){this.transport.close();this.transport=fjs.fdp.transport.TransportFactory.getTransport(this.ticket,this.node,this.serverHost,this.type);this.addTransportEvents();clearTimeout(this.versionsTimeoutId);this.startSync(this.syncFeeds)};fjs.fdp.SyncManager.prototype.onStorage=function(a){if(-1<a.key.indexOf("lsp_")){var b=a.key.replace("lsp_","");if((new fjs.api.TabsSynchronizer).isMaster)if(a=fjs.utils.JSON.parse(a.newValue),"clientSync"==b)this.onClientSync(a);
else this.transport.send({type:b,data:a})}};fjs.fdp.SyncManager.prototype.addTransportEvents=function(){var a=this;this.transport.addEventListener("message",function(b){fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()&&(new fjs.api.TabsSynchronizer).isMaster&&fjs.fdp.transport.LocalStorageTransport.masterSend("message",b);switch(b.type){case "sync":a.onSync(b.data);break;case "ticket":a.fireEvent("ticket",b);a.startSync(a.syncFeeds);break;default:a.fireEvent(b.type,b)}});this.transport.addEventListener("error",
function(b){fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()&&(new fjs.api.TabsSynchronizer).isMaster&&fjs.fdp.transport.LocalStorageTransport.masterSend("error",b);switch(b.type){default:a.fireEvent(b.type,b)}})};fjs.fdp.SyncManager.prototype.initDataBase=function(a){if(this.db){for(var b=this,d=0;d<this.config.DB.tables.length;d++){var e=this.config.DB.tables[d];this.db.declareTable(e.name,e.key,e.indexes)}this.db.open(this.config.DB.name,this.config.DB.version,function(d){d||
(b.db=null);b.finishInitialization(a);fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()&&b.tabsSyncronizer.isMaster&&b.db.deleteByKey("tabsync",null)})}else this.finishInitialization(a)};fjs.fdp.SyncManager.prototype.getDataForFeeds=function(a,c){var d=this;fjs.utils.Core.asyncForIn(a,function(a,c,g){d.getFeedData(c,function(a){d.fireEvent(a.feed,a);a.eventType==b.eventTypes.FEED_COMPLETE&&g()})},function(){c()})};fjs.fdp.SyncManager.prototype.finishInitialization=function(a){function c(){0<
d.suspendFeeds.length&&d.startSync(d.suspendFeeds);d.suspendFeeds=[];d.suspendClientFeeds=[];d.status=b.states.READY;a()}var d=this;if(this.suspendLoginData){var e=this.suspendLoginData;this.db?this.db.clear(function(){d.transport.send({type:"SFLogin",data:e});c()}):(d.transport.send({type:"SFLogin",data:e}),c());this.suspendLoginData=null}else 0<this.suspendFeeds.length?(this.fireEvent(null,{eventType:b.eventTypes.SYNC_START}),this.getDataForFeeds(this.suspendFeeds,function(){0<d.suspendClientFeeds.length?
d.getDataForFeeds(d.suspendClientFeeds,function(){d.fireEvent(null,{eventType:b.eventTypes.SYNC_COMPLETE});c()}):(d.fireEvent(null,{eventType:b.eventTypes.SYNC_COMPLETE}),c())})):0<this.suspendClientFeeds.length?(this.fireEvent(null,{eventType:b.eventTypes.SYNC_START}),this.getDataForFeeds(this.suspendClientFeeds,function(){d.fireEvent(null,{eventType:b.eventTypes.SYNC_COMPLETE});c()})):c()};fjs.fdp.SyncManager.prototype.startSync=function(a){var b={},d=this;a?fjs.utils.Core.asyncForIn(a,function(a,
f,g){d.getVersions(f,b,function(){g()})},function(){d.transport.send({type:"synchronize",data:{versions:b}})}):Error("You must set feeds names array")};fjs.fdp.SyncManager.prototype.saveVersions=function(a,b,d){this.db&&(void 0!==d&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.api.TabsSynchronizer).isMaster))&&this.db.insertOne("versions",{feedSource:a+"_"+b,feedName:a,source:b,version:d})};fjs.fdp.SyncManager.prototype.getVersions=function(a,b,d){var e=[],f=this;
this.versions[a]?setTimeout(function(){d(b[a]=f.versions[a])},0):this.db?this.db.selectByIndex("versions",{feedName:a},function(a){e.push(a.source+"@"+a.version)},function(){f.versions[a]=b[a]=e.join(",");d(b[a])}):setTimeout(function(){d(f.versions[a]=b[a]="")},0)};fjs.fdp.SyncManager.prototype.lazySyncProcessItems=function(a,c,d){for(var e=[],f=0;f<a.length;f++){var g=a[f],h=g.xef001type||b.eventTypes.ENTRY_CHANGE,i=g.xpid=g.xpid?g.xpid:d?d+"_"+g.xef001id:g.xef001id;delete g.xef001type;g.source=
d;g={eventType:h,feed:c,xpid:i,entry:g};if(this.db)switch(h){case b.eventTypes.ENTRY_CHANGE:e.push(g.entry);break;case b.eventTypes.ENTRY_DELETION:(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.api.TabsSynchronizer).isMaster)&&this.db.deleteByKey(c,g.xpid,null);break;default:fjs.utils.Console.error("Incorrect item change type: "+h+" for Lazy sync")}this.fireEvent(c,g)}this.db&&(0<e.length&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.api.TabsSynchronizer).isMaster))&&
this.db.insertArray(c,e,null)};fjs.fdp.SyncManager.prototype.fullSyncProcessItems=function(a,c,d){for(var e=this,f=[],g=0;g<a.length;g++){var h=a[g],i=h.xef001type||b.eventTypes.ENTRY_CHANGE;h.source=d;if(i===b.eventTypes.ENTRY_CHANGE){var j=h.xpid=h.xpid?h.xpid:d?d+"_"+h.xef001id:h.xef001id;delete h.xef001type;i={eventType:i,feed:c,xpid:j,entry:h};f.push(h);this.fireEvent(c,i)}else fjs.utils.Console.error("Incorrect item change type: "+i+" for Full sync")}this.db&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||
(new fjs.api.TabsSynchronizer).isMaster)&&this.db.deleteByIndex(c,{source:d},function(){e.db.insertArray(c,f,null)})};fjs.fdp.SyncManager.prototype.keepSyncProcessItems=function(a,c,d){for(var e=[],f=[],g=[],h=this,i=0;i<a.length;i++){var j=a[i],k=j.xef001type||b.eventTypes.ENTRY_CHANGE,l=j.xpid=j.xpid?j.xpid:d?d+"_"+j.xef001id:j.xef001id;delete j.xef001type;j.source=d;j={eventType:k,feed:c,xpid:l,entry:j};if(this.db)switch(k){case b.eventTypes.ENTRY_CHANGE:e.push(j.entry);g.push(j.entry.xpid);break;
case b.eventTypes.ENTRY_KEEP:f.push(j.entry.xpid);break;default:fjs.utils.Console.error("Incorrect item change type: "+k+" for Keep sync")}this.fireEvent(c,j)}this.db&&(0<e.length&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.api.TabsSynchronizer).isMaster))&&this.db.selectByIndex(c,{source:d},function(a){0>f.indexOf(a.xpid)&&0>g.indexOf(a.xpid)&&h.db.deleteByKey(c,a.xpid,null)},function(){h.db.insertArray(c,e,null)})};fjs.fdp.SyncManager.prototype.processFeedData=
...
{eventType:b.eventTypes.SYNC_START});for(a=0;a<this.syncs.length;a++)for(var d in this.syncs[a])this.syncs[a].hasOwnProperty(d)&&this.processFeedData(d,this.syncs[a][d]);c.syncs=[];this.syncTimeoutId=setTimeout(function(){c.fireEvent(null,{eventType:b.eventTypes.SYNC_COMPLETE});c.syncTimeoutId=null},100)}else fjs.utils.Console.log("!!!!Double onSync!!",this.syncs[0],a)};fjs.fdp.SyncManager.prototype.sendAction=function(a,b,d){d.action=b;this.transport.send({type:"action",data:{feedName:a,actionName:b,
parameters:d}})};fjs.fdp.SyncManager.prototype.onClientSync=function(a,b){fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()&&(new fjs.api.TabsSynchronizer).isMaster?fjs.fdp.transport.LocalStorageTransport.masterSend("message",{type:"sync",data:a}):fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()&&!b&&fjs.fdp.transport.LocalStorageTransport.masterSend("clientSync",a);if(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.api.TabsSynchronizer).isMaster)this.onSync(a)};
fjs.fdp.SyncManager.prototype.getAuthTicket=function(){var a=this;this.state==b.states.READY&&this.db?this.db.clear(function(){a.fireEvent("authError",{type:"authError",message:"Auth ticket wrong or expired"})}):a.fireEvent("authError",{type:"authError",message:"Auth ticket wrong or expired"})};fjs.fdp.SyncManager.prototype.addFeedListener=function(a,c,d){var e=this;0>this.feeds.indexOf(a)&&this.feeds.push(a);this.superClass.addEventListener.apply(this,arguments);this.status!=b.states.READY?d?0>this.suspendClientFeeds.indexOf(a)&&
this.suspendClientFeeds.push(a):(0>this.suspendFeeds.indexOf(a)&&this.suspendFeeds.push(a),0>this.syncFeeds.indexOf(a)&&this.syncFeeds.push(a)):0>this.syncFeeds.indexOf(a)?(d||(this.syncFeeds.push(a),0>this.versionsFeeds.indexOf(a)&&this.versionsFeeds.push(a),null!=this.versionsTimeoutId&&(clearTimeout(this.versionsTimeoutId),this.versionsTimeoutId=null),this.versionsTimeoutId=setTimeout(function(){e.startSync(e.versionsFeeds);e.versionsFeeds=[];e.versionsTimeoutId=null},100)),this.fireEvent(a,{eventType:b.eventTypes.SYNC_START,
feed:a},c),this.getFeedData(a,function(d){d.eventType==b.eventTypes.FEED_COMPLETE&&e.fireEvent(a,{eventType:b.eventTypes.SYNC_COMPLETE,feed:a},c);c(d)})):0<=this.syncFeeds.indexOf(a)&&this.states==b.READY&&(this.fireEvent(a,{eventType:b.eventTypes.SYNC_START,feed:a},c),this.getFeedData(a,function(d){d.eventType==b.eventTypes.FEED_COMPLETE&&e.fireEvent(a,{eventType:b.eventTypes.SYNC_COMPLETE,feed:a},c);c(d)}))};fjs.fdp.SyncManager.prototype.removeFeedListener=function(a,b,d){var e;(e=0<=this.feeds.indexOf(a))&&
this.feeds.splice(e,1);this.superClass.removeEventListener.apply(this,arguments);d||(-1<(e=this.syncFeeds.indexOf(a))&&this.syncFeeds.splice(e,1),this.listeners[a]&&0==this.listeners[a].length&&this.transport.send({type:"forget",data:{feedName:a}}))};fjs.fdp.SyncManager.prototype.getFeedData=function(a,c){var d=this;this.fireEvent(a,{eventType:b.eventTypes.FEED_START,syncType:"F",feed:a},c);this.db?this.db.selectAll(a,function(e){d.fireEvent(a,{eventType:b.eventTypes.ENTRY_CHANGE,feed:a,xpid:e.xpid,
entry:e},c)},function(){d.fireEvent(a,{eventType:b.eventTypes.FEED_COMPLETE,feed:a},c)}):this.fireEvent(a,{eventType:b.eventTypes.FEED_COMPLETE,feed:a},c)};fjs.fdp.SyncManager.prototype.fireEvent=function(a,b,d){if(d)d(b);else if(a)this.superClass.fireEvent.call(this,a,b);else for(var e in this.listeners)this.listeners.hasOwnProperty(e)&&-1<this.feeds.indexOf(e)&&this.superClass.fireEvent.call(this,e,b)};fjs.fdp.SyncManager.prototype.loadNext=function(a,b,d){var e=fjs.utils.JSON.stringify(b),b=this.historyversions[a];
b||(b={},this.historyversions[a]=b);if(b[e])this.processHistoryVersions(a,e,d,b[e],function(){});else{var f=this,g=[];this.fillHistoryVersions(a,e,d,g,function(){f.processHistoryVersions(a,e,d,g,function(){})})}};fjs.fdp.SyncManager.prototype.fillHistoryVersions=function(a,b,d,e,f){if(this.db){var g=this;this.db.selectByIndex("historyversions",{feedName:a,filter:b},function(a){e.push(a.source+":"+a.version)},function(){0==e.length?g.fillEmptyHistoryVersionsFromFeed(a,e,f):f()})}};fjs.fdp.SyncManager.prototype.fillEmptyHistoryVersionsFromFeed=
function(a,b,d){this.db&&this.db.selectByIndex("versions",{feedName:a},function(a){b.push(a.source+":0")},d)};fjs.fdp.SyncManager.prototype.processHistoryVersions=function(a,b,d,e,f){0==e.length?f&&f(!1):(this.historyversions[a][b]=e,b={"s.limit":d,"sh.filter":b,"sh.versions":e.join("@")},this.transport.send({type:"loadNext",data:{feedName:a,data:b}}),f&&f(!0))};fjs.fdp.SyncManager.prototype.saveHistoryVersions=function(a,b,d,e){if(this.db&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||
(new fjs.api.TabsSynchronizer).isMaster))if(this.db.insertOne("historyversions",{feedSourceFilter:a+"_"+b+"_filter",feedName:a,source:b,filter:d,version:e}),b=this.historyversions[a])delete b[d],b||delete this.historyversions[a]};fjs.fdp.SyncManager.prototype.saveEmptyHistoryVersions=function(a,b){if(this.db&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.api.TabsSynchronizer).isMaster))this.db.insertOne("historyversions",{feedName:a,filter:b,version:"-1"}),delete this.historyversions[a]};
fjs.fdp.SyncManager.prototype.logout=function(){this.node=this.ticket=null;this.getAuthTicket();this.status=b.states.NOT_INITIALIZED};fjs.fdp.SyncManager.prototype.SFLogin=function(a){var c=this;this.status==b.states.READY&&this.db?this.db.clear(function(){c.transport.send({type:"SFLogin",data:a})}):this.db?this.suspendLoginData=a:this.transport.send({type:"SFLogin",data:a})}})();namespace("fjs.fdp");
