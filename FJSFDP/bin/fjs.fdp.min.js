'use strict';namespace("fjs.db");fjs.db.IndexedDBProvider=function(){this.indexedDB=self.indexedDB||self.mozIndexedDB||self.webkitIndexedDB||self.msIndexedDB;this.IDBTransaction=self.IDBTransaction||self.webkitIDBTransaction||self.msIDBTransaction;this.IDBKeyRange=self.IDBKeyRange||self.webkitIDBKeyRange||self.msIDBKeyRange;this.db=null;this.tables={};this.state=-1};fjs.db.IndexedDBProvider.check=function(){return!(!self.indexedDB&&!self.mozIndexedDB&&!self.webkitIndexedDB&&!self.msIndexedDB)};
fjs.db.IndexedDBProvider.prototype.open=function(b,a,c){this.state=0;var d=this.indexedDB.open(b,a),e=this;d.onerror=function(d){fjs.utils.Console.error("Error: can't open indexedDB ("+b+", "+a+")",d);c(null)};var f=function(a){fjs.utils.Console.error("Database error: "+a.target.errorCode,a);c(null)};d.onsuccess=function(){e.db=d.result;e.db.onerror=f;e.state=1;c&&c(e.db)};d.onupgradeneeded=function(a){var b=e.db=a.target.result;for(a.target.transaction.onerror=b.onerror=f;0<b.objectStoreNames.length;)b.deleteObjectStore(b.objectStoreNames[0]);
for(var c in e.tables)e.tables.hasOwnProperty(c)&&(a=e.tables[c],e.createTable(c,a.key,a.indexes))}};fjs.db.IndexedDBProvider.prototype.declareTable=function(b,a,c){b=this.tables[b]={key:a,indexes:c};fjs.utils.Browser.isIE()&&c&&(b.multipleIndexes=this.IEDeclareMultipleIndexes(c))};
...
fjs.db.LocalStorageDbProvider=function(){this.dbInfo=null;this.tables={};this.dbData={};this.indexes={};this.state=-1};
...
fjs.db.LocalStorageDbProvider.prototype.open=function(b,a,c){var d=fjs.utils.LocalStorage;try{var e,f=this;this.state=0;if(this.dbInfo=fjs.utils.JSON.parse(d.get("DB_"+b)))if(a>this.dbInfo.version){for(var g=0;g<this.dbInfo.tables.length;g++)e=this.dbInfo.tables[g],d.remove(e);this.dbInfo.tables=null;this.dbInfo.version=a;this.createTables();d.set("DB_"+b,fjs.utils.JSON.stringify(this.dbInfo))}else for(b=0;b<this.dbInfo.tables.length;b++)e=this.dbInfo.tables[b],g=this.dbData[e]=fjs.utils.JSON.parse(d.get(e))||
{},this.createIndexes(this.getRealTableName(e),g);else this.dbInfo={name:b,version:a},this.createTables(),d.set("DB_"+b,fjs.utils.JSON.stringify(this.dbInfo));this.state=1;c&&setTimeout(function(){c(f)},0)}catch(h){fjs.utils.Console.error("Error: can't create localStorage DB",h),c&&setTimeout(function(){c(null)},0)}};fjs.db.LocalStorageDbProvider.check=function(){return fjs.utils.LocalStorage.check()};fjs.db.LocalStorageDbProvider.prototype.createTable=function(){};
...
fjs.db.LocalStorageDbProvider.prototype.declareTable=function(b,a,c){this.tables[b]={name:b,key:a,indexes:c}};fjs.db.LocalStorageDbProvider.prototype.insertOne=function(b,a,c){var d=this.getLSTableName(b);this.createIndexes(b,[a]);this.dbData[d][a[this.tables[b].key]]=a;fjs.utils.LocalStorage.set(d,fjs.utils.JSON.stringify(this.dbData[d]));c&&setTimeout(c,0)};
fjs.db.LocalStorageDbProvider.prototype.insertArray=function(b,a,c){var d=this.getLSTableName(b);this.createIndexes(b,a);for(var e=0;e<a.length;e++){var f=a[e];this.dbData[d][f[this.tables[b].key]]=f}fjs.utils.LocalStorage.set(d,fjs.utils.JSON.stringify(this.dbData[d]));c&&setTimeout(c,0)};
fjs.db.LocalStorageDbProvider.prototype.deleteByKey=function(b,a,c){b=this.getLSTableName(b);null!=a&&this.dbData[b]?delete this.dbData[b][a]:(this.dbData[b]={},this.indexes[b]={});fjs.utils.LocalStorage.set(b,fjs.utils.JSON.stringify(this.dbData[b]));c&&setTimeout(c,0)};
...
setTimeout(function(){d(e)},0)},0)}(a,m);else setTimeout(function(){d(e)},0)}else this.selectAll(b,c,d)};fjs.db.LocalStorageDbProvider.prototype.selectByKey=function(b,a,c){b=this.getLSTableName(b);setTimeout(c(this.dbData[b][a]),0)};fjs.db.LocalStorageDbProvider.prototype.clear=function(b){for(var a in this.tables)if(this.tables.hasOwnProperty(a)){var c=this.getLSTableName(a);fjs.utils.LocalStorage.remove(c);delete this.dbData[c];delete this.indexes[c]}b&&setTimeout(b,0)};
fjs.db.LocalStorageDbProvider.prototype.deleteByIndex=function(b,a,c){var d=[],e=[],f=[],b=this.getLSTableName(b),g=[],h,i;if(null==a){for(var j in this.dbData[b])this.dbData[b].hasOwnProperty(j)&&(d.push(this.dbData[b][j]),delete this.dbData[b][j]);this.indexes[b]={};fjs.utils.LocalStorage.set(b,fjs.utils.JSON.stringify(this.dbData[b]))}else{for(var k in a)a.hasOwnProperty(k)&&e.push(k);if(1<e.length){e.sort();h=e.join("_");for(g=0;g<e.length;g++)f.push(a[e[g]]);i=f.join("_")}else 1==e.length&&(h=
e[0],i=a[h]);if(g=this.indexes[b]&&this.indexes[b][h]&&this.indexes[b][h][i])for(a=0;a<g.length;a++)d.push(this.dbData[b][g[a]]),delete this.dbData[b][g[a]]}setTimeout(function(){c(d)},0)};namespace("fjs.db");
fjs.db.DBFactory=function(b){if(this.constructor.__instance)return this.constructor.__instance;this.constructor.__instance=this;this.config=b;this._dbRegister={indexedDB:fjs.db.IndexedDBProvider,webSQL:fjs.db.WebSQLProvider,localStorage:fjs.db.LocalStorageDbProvider};this.currentDB=null;this.currentDBIndex=-1};
fjs.db.DBFactory.prototype.getDB=function(b){if(!this.currentDB||b)for(var b=this.config.DB.dbProviders,a=this.currentDBIndex+1;a<b.length;a++)if(this._dbRegister[b[a]].check()){this.currentDB=new this._dbRegister[b[a]];this.currentDBIndex=a;break}return this.currentDB};namespace("fjs.fdp.model");fjs.fdp.model.EntryModel=function(b){this.xpid=this.xef001iver=this.xef001id=null;this.fill(b)};
fjs.fdp.model.EntryModel.prototype.fill=function(b){var a={},c=0;if(b)for(var d in b)b.hasOwnProperty(d)&&this[d]!=b[d]&&(this[d]=b[d],a[d]=this[d],c++);return c?(a.xpid||(a.xpid=this.xpid),a):null};namespace("fjs.fdp.model");
...
fjs.fdp.model.ProxyModel.prototype.clear=function(){this.changes={};for(var b in this.items)this.items.hasOwnProperty(b)&&(this.changes[b]={type:"delete",entry:null,xpid:b},delete this.items[b]);this.onSyncComplete()};
fjs.fdp.model.ProxyModel.prototype.onSyncEvent=function(b){var a=fjs.fdp.SyncManager.eventTypes;switch(b.eventType){case a.CLEAR:this.clear();break;case a.ENTRY_CHANGE:this.onEntryChange(b);break;case a.ENTRY_DELETION:this.onEntryDeletion(b);break;case a.ENTRY_KEEP:this.onEntryKeep(b);break;case a.SOURCE_COMPLETE:this.onSourceComplete(b);break;case a.SYNC_COMPLETE:this.onSyncComplete(b)}};
...
fjs.fdp.model.ClientFeedProxyModel.prototype.sendAction=function(b,a,c,d){"push"===a||"delete"===a?(b=this.createSyncData(a,c),this.sm.onClientSync(fjs.utils.JSON.stringify(b),d)):fjs.fdp.model.ProxyModel.prototype.sendAction.call(this,this.feedName,a,c)};fjs.fdp.model.ClientFeedProxyModel.prototype.onEntryDeletion=function(b){b.feed==this.feedName&&delete this.items[b.xpid];this.fillDeletion(b.xpid,b.feed)};
...
fjs.fdp.model.ClientFeedProxyModel.prototype.onEntryChange=function(b){var a=this.items[b.xpid],c=this.prepareEntry(b.entry,b.feed,b.xpid);if(a)(a=a.fill(c))&&this.fillChange(b.xpid,a,b.feed);else{if(b.feed==this.clientFeedName&&this.clientFeedName!=this.feedName)return;"delete"!=this.fillChange(b.xpid,c,b.feed).type&&(this.items[b.xpid]=new fjs.fdp.model.EntryModel(c))}this.keepEntries[b.xpid]=b};namespace("fjs.fdp.model");
fjs.fdp.model.MyCallsClientProxyModel=function(b,a){fjs.fdp.model.ClientFeedProxyModel.call(this,b,a);this.callLogByHTCallId={}};fjs.fdp.model.MyCallsClientProxyModel.extend(fjs.fdp.model.ClientFeedProxyModel);
fjs.fdp.model.MyCallsClientProxyModel.prototype.onEntryChange=function(b){var a=this.items[b.xpid],c=this.prepareEntry(b.entry,b.feed,b.xpid);if(a)(a=a.fill(c))&&this.fillChange(b.xpid,a,b.feed);else{if(b.feed==this.clientFeedName&&this.clientFeedName!=this.feedName)return;"delete"!=this.fillChange(b.xpid,c,b.feed).type&&(this.items[b.xpid]=new fjs.fdp.model.EntryModel(c))}this.keepEntries[b.xpid]=b};
fjs.fdp.model.MyCallsClientProxyModel.prototype.onEntryDeletion=function(b){if(b.feed==this.feedName){var a=this.items[b.xpid];this.sendAction(this.clientFeedName,"delete",{xpid:b.xpid},!0);a&&(this.callLogByHTCallId[a.htCallId]=a.mycallsclient_callLog,delete this.items[b.xpid])}this.fillDeletion(b.xpid,b.feed)};
fjs.fdp.model.MyCallsClientProxyModel.prototype.onSyncComplete=function(){for(var b=Object.keys(this.items),a=0;a<b.length;a++){var c=b[a],d=this.callLogByHTCallId[this.items[c].htCallId];d&&this.sendAction(this.clientFeedName,"push",{callLog:d,xpid:c})}this.changes&&this.fireEvent({feed:this.clientFeedName,changes:this.changes});this.changes=null;this.callLogByHTCallId={}};
...
this;this._currentRequest=this.sendRequest(this.url+this.SF_LOGIN_PATH,b,function(b,d,e){try{if(e){var f=fjs.utils.JSON.parse(d),g,h;f&&(g=f.Auth)&&(h=f.node)?(a.fireEvent("message",{type:"node",data:{nodeId:a.node=h}}),a.fireEvent("message",{type:"ticket",data:{ticket:a.ticket=g}})):a.fireEvent("error",{type:"authError",message:"Can't get ticket or node"})}else if(403==b.status||401==b.status)a.fireEvent("error",{type:"authError",message:d?d.replace("Error=",""):"Wrong auth data"});a.handleRequestErrors(b,
e)}catch(i){a.fireEvent("error",{type:"authError",message:"Can't get ticket or node"})}})};fjs.fdp.transport.AJAXTransport.prototype.loadNext=function(b){var a=b.data,c=this;this._currentRequest=this.sendRequest(this.url+"/v1/history/"+a.feedName,a.data,function(b,e,f){if(f){var e=fjs.utils.JSON.parse(e),g={};g[a.feedName]=e;for(var h in e)e.hasOwnProperty(h)&&(e[h].filter=a.data["sh.filter"]);c.fireEvent("message",{type:"sync",data:g})}c.handleRequestErrors(b,f)})};fjs.fdp.transport.AJAXTransport.prototype._forgetFeed=
function(b){0>this._forgetFeeds.indexOf(b)&&this._forgetFeeds.push(b)};fjs.fdp.transport.AJAXTransport.prototype.parseClientRegistryResponse=function(b){for(var b=b.split("\n"),a={},c=0;c<b.length;c++)if(-1<b[c].indexOf("=")){var d=b[c].split("="),e=d[0].trim(),d=d[1].trim();e&&d&&(a[e]=d)}return a};fjs.fdp.transport.AJAXTransport.prototype.requestClientRegistry=function(b){var a=this;this._currentRequest=this.sendRequest(this.url+this.CLIENT_REGISRY_PATH,{},function(c,d,e){if(e){a._clientRegistryFailedCount=
0;var f=a.parseClientRegistryResponse(d).node;f&&(a.fireEvent("message",{type:"node",data:{nodeId:a.node=f}}),b(d))}else 403==c.status||401==c.status?(b(!1),a.fireEvent("error",{type:"authError",message:"Auth ticket wrong or expired"})):10>a._clientRegistryFailedCount?(a._clientRegistryFailedCount++,a.requestClientRegistry(b)):b(!1);a.handleRequestErrors(c,e)})};fjs.fdp.transport.AJAXTransport.prototype.requestVersionscache=function(){var b=this;this._currentRequest=this._currentVersioncache=this.sendRequest(this.url+
this.VERSIONSCACHE_PATH,null,function(a,c,d){d?(b._clientRegistryFailedCount=0,(c=b.parseVersionsResponse(c))?b.syncRequest(c):b.requestVersionscache()):401==a.status||403==a.status?b.requestClientRegistry(function(a){a&&b.requestVersionscache()}):a.aborted||(5>b._clientRegistryFailedCount?(b.requestVersionscache(),b._clientRegistryFailedCount++):5<=b._clientRegistryFailedCount&&setTimeout(function(){b.requestVersionscache()},5E3));b.handleRequestErrors(a,d)})};fjs.fdp.transport.AJAXTransport.prototype.syncRequest=
function(b){var a=this;this._currentRequest=this.sendRequest(this.url+this.SYNC_PATH,b,function(b,d,e){e&&(d=fjs.utils.JSON.parse(d),a.fireEvent("message",{type:"sync",data:d}));a.requestVersionscache();a.handleRequestErrors(b,e)})};fjs.fdp.transport.AJAXTransport.prototype.parseVersionsResponse=function(b){var b=b.split(";"),a={},c=0,d;if(b){this.fireEvent("message",{type:"fdpTime",data:b[0]});for(var e=2;e<b.length-1;e++)b[e]&&(0>(d=this._forgetFeeds.indexOf(b[e]))?a[b[e]]="":this._forgetFeeds.splice(d,
1),c++)}return c?a:null};fjs.fdp.transport.AJAXTransport.prototype.requestVersions=function(b){var a=this;null!=this._currentVersioncache&&this.ajax.abort(this._currentVersioncache);if(!this.node)return this.requestClientRegistry(function(c){c&&a.requestVersions(b)});this._currentVersioncache=this._currentRequest=this.sendRequest(this.url+this.VERSIONS_PATH,b,function(c,d,e){e?(d=a.parseVersionsResponse(d))?a.syncRequest(d):a.requestVersionscache():(401==c.status||403==c.status)&&a.requestClientRegistry(function(c){c&&
a.requestVersions(b)});a.handleRequestErrors(c,e)})};fjs.fdp.transport.AJAXTransport.prototype.sendAction=function(b,a,c){var d=this,e={action:a},f;for(f in c)c.hasOwnProperty(f)&&(e["a."+f]=c[f]);this.sendRequest(this.url+"/v1/"+b,e,function(e,f,i){d.handleRequestErrors(e,i);i||(401==e.status||403==e.status)&&d.requestClientRegistry(function(e){e&&d.sendAction(b,a,c)})})};fjs.fdp.transport.AJAXTransport.prototype.close=function(){this.closed=!0;this._currentRequest&&this.ajax.abort(this._currentRequest)}})();
(function(){namespace("fjs.fdp");fjs.fdp.transport.XHRTransport=function(b,a,c,d){fjs.fdp.transport.AJAXTransport.call(this,b,a,c,d);this.ajax=new fjs.ajax.XHRAjax};fjs.fdp.transport.XHRTransport.extend(fjs.fdp.transport.AJAXTransport);fjs.fdp.transport.XHRTransport.prototype.sendRequest=function(b,a,c){if(this.closed)return null;fjs.utils.Browser.isSafari()&&(b+=(/\?/.test(b)?"&safariNoCa\u0441he=":"?safariNoCa\u0441he=")+Date.now());a=a||{};a.t=this.type;a.alt="j";a=this.ajax.send("post",b,{Authorization:"auth="+
this.ticket,node:this.node||""},a,c);a.url=b;return a}})();
...
(function(){namespace("fjs.fdp.transport");fjs.fdp.transport.LocalStorageTransport=function(){var b=this;fjs.fdp.transport.FDPTransport.call(this);this.tabsSynchronizer=new fjs.api.TabsSynchronizer;this.onStorage=function(a){if(-1<a.key.indexOf("lsp_")){var c=a.key.replace("lsp_","");b.fireEvent(c,fjs.utils.JSON.parse(a.newValue))}};fjs.utils.Browser.isIE11()?(this.tabsSynchronizer.addEventListener("lsp_message",this.onStorage),this.tabsSynchronizer.addEventListener("lsp_error",this.onStorage),this.tabsSynchronizer.addEventListener("lsp_clientSync",
this.onStorage),this.tabsSynchronizer.addEventListener("lsp_action",this.onStorage)):window.addEventListener("storage",this.onStorage,!1)};fjs.fdp.transport.LocalStorageTransport.extend(fjs.fdp.transport.FDPTransport);fjs.fdp.transport.LocalStorageTransport.prototype.send=function(b){fjs.utils.Browser.isIE11()?this.tabsSynchronizer.setSyncValue("lsp_"+b.type,fjs.utils.JSON.stringify(b.data)):(b.data.t=Date.now(),fjs.utils.LocalStorage.set("lsp_"+b.type,fjs.utils.JSON.stringify(b.data)))};fjs.fdp.transport.LocalStorageTransport.prototype.close=
function(){window.removeEventListener("storage",this.onStorage,!1);this.tabsSynchronizer.removeEventListener("lsp_message",this.onStorage);this.tabsSynchronizer.removeEventListener("lsp_error",this.onStorage);this.tabsSynchronizer.removeEventListener("lsp_clientSync",this.onStorage);this.tabsSynchronizer.removeEventListener("lsp_action",this.onStorage)};fjs.fdp.transport.LocalStorageTransport.masterSend=function(b,a){fjs.utils.Browser.isIE11()&&(new fjs.api.TabsSynchronizer).setSyncValue("lsp_"+b,
fjs.utils.JSON.stringify(a));a.t=Date.now();fjs.utils.LocalStorage.set("lsp_"+b,fjs.utils.JSON.stringify(a))}})();
...
(function(){namespace("fjs.fdp");fjs.fdp.SyncManager=function(a){if(this.constructor.__instance)return this.constructor.__instance;this.constructor.__instance=this;fjs.EventsSource.call(this);this.dbFactory=new fjs.db.DBFactory(a);this.db=this.dbFactory.getDB();this.syncs=[];this.runtimefeeds=["mycalldetails","mycalls"];this.status=b.states.NOT_INITIALIZED;this.config=a;this.syncTimeoutId=this.transport=null;this.versions={};this.historyversions={};this.node=this.ticket=null;this.serverHost=this.config.SERVER.serverURL;
this.type=this.config.CLIENT.type;this.syncFeeds=[];this.suspendFeeds=[];this.suspendActions=[];this.suspendClientSyncs=[];this.suspendClientFeeds=[];this.versionsFeeds=[];this.versionsTimeoutId=null;this.feeds=[];this.suspendLoginData=this.tabsSyncronizer=null};fjs.fdp.SyncManager.extend(fjs.EventsSource);var b=fjs.fdp.SyncManager;fjs.fdp.SyncManager.states={NOT_INITIALIZED:-1,INITIALIZATION:0,READY:1};fjs.fdp.SyncManager.eventTypes={SYNC_START:"syncStart",SYNC_COMPLETE:"syncComplete",FEED_START:"feedStart",
FEED_COMPLETE:"feedComplete",SOURCE_START:"sourceStart",SOURCE_COMPLETE:"sourceComplete",ENTRY_CHANGE:"push",ENTRY_DELETION:"delete",ENTRY_KEEP:"keep",CLEAR:"clear"};fjs.fdp.SyncManager.syncTypes={FULL:"F",KEEP:"I",LAZY:"L",HISTORY:"H"};fjs.fdp.SyncManager.prototype.init=function(a,c,d){var e=this;this.status=b.states.INITIALIZATION;this.ticket=a;this.node=c;fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()&&(this.tabsSyncronizer=new fjs.api.TabsSynchronizer,fjs.utils.Browser.isIE11()?
(this.tabsSyncronizer.addEventListener("lsp_clientSync",function(a){e.onStorage(a)}),this.tabsSyncronizer.addEventListener("lsp_message",function(a){e.onStorage(a)}),this.tabsSyncronizer.addEventListener("lsp_error",function(a){e.onStorage(a)}),this.tabsSyncronizer.addEventListener("lsp_action",function(a){e.onStorage(a)})):self.addEventListener("storage",function(a){e.onStorage(a)},!1),this.tabsSyncronizer.addEventListener("master_changed",function(){e.onMasterChanged()}));this.transport=fjs.fdp.transport.TransportFactory.getTransport(this.ticket,
this.node,this.serverHost,this.type);this.addTransportEvents();this.initDataBase(d)};fjs.fdp.SyncManager.prototype.onMasterChanged=function(){this.transport.close();this.transport=fjs.fdp.transport.TransportFactory.getTransport(this.ticket,this.node,this.serverHost,this.type);this.addTransportEvents();clearTimeout(this.versionsTimeoutId);this.startSync(this.syncFeeds)};fjs.fdp.SyncManager.prototype.onStorage=function(a){if(-1<a.key.indexOf("lsp_")){var b=a.key.replace("lsp_","");if((new fjs.api.TabsSynchronizer).isMaster)if(a=
fjs.utils.JSON.parse(a.newValue),"clientSync"==b)this.onClientSync(a.data);else this.transport.send({type:b,data:a})}};fjs.fdp.SyncManager.prototype.addTransportEvents=function(){var a=this;this.transport.addEventListener("message",function(c){fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()&&(new fjs.api.TabsSynchronizer).isMaster&&fjs.fdp.transport.LocalStorageTransport.masterSend("message",c);switch(c.type){case "sync":a.onSync(c.data);break;case "ticket":a.fireEvent("ticket",
c);a.startSync(a.syncFeeds);break;case b.eventTypes.CLEAR:a.fireEvent(null,{eventType:b.eventTypes.CLEAR});break;default:a.fireEvent(c.type,c)}});this.transport.addEventListener("error",function(b){fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()&&(new fjs.api.TabsSynchronizer).isMaster&&fjs.fdp.transport.LocalStorageTransport.masterSend("error",b);switch(b.type){default:a.fireEvent(b.type,b)}})};fjs.fdp.SyncManager.prototype.initDataBase=function(a){this.db?this.openDB(a):this.finishInitialization(a)};
fjs.fdp.SyncManager.prototype.openDB=function(a){for(var b=this,d=0;d<this.config.DB.tables.length;d++){var e=this.config.DB.tables[d];this.db.declareTable(e.name,e.key,e.indexes)}this.db.open(this.config.DB.name,this.config.DB.version,function(d){d||(b.db=b.dbFactory.getDB(b.db),b.openDB(a));b.finishInitialization(a);fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()&&b.tabsSyncronizer.isMaster&&b.db.deleteByKey("tabsync",null)})};fjs.fdp.SyncManager.prototype.getDataForFeeds=function(a,
c){var d=this;fjs.utils.Core.asyncForIn(a,function(a,c,g){d.getFeedData(c,function(a){d.fireEvent(a.feed,a);a.eventType==b.eventTypes.FEED_COMPLETE&&g()})},function(){c()})};fjs.fdp.SyncManager.prototype.finishInitialization=function(a){function c(){0<d.suspendFeeds.length&&d.startSync(d.suspendFeeds);d.suspendFeeds=[];if(0<d.suspendClientSyncs.length)for(var c=0;c<d.suspendClientSyncs.length;c++)d.onSync(d.suspendClientSyncs[c]);d.suspendClientSyncs=[];if(0<d.suspendActions.length)for(c=0;c<d.suspendActions.length;c++)d.transport.send(d.suspendActions[c]);
d.suspendActions=[];d.suspendFeeds=[];d.suspendClientFeeds=[];d.status=b.states.READY;a()}var d=this;if(this.suspendLoginData){var e=this.suspendLoginData;this.db?this.db.clear(function(){d.transport.send({type:"SFLogin",data:e});c()}):(d.transport.send({type:"SFLogin",data:e}),c());this.suspendLoginData=null}else 0<this.suspendFeeds.length?(this.fireEvent(null,{eventType:b.eventTypes.SYNC_START}),this.getDataForFeeds(this.suspendFeeds,function(){0<d.suspendClientFeeds.length?d.getDataForFeeds(d.suspendClientFeeds,
function(){d.fireEvent(null,{eventType:b.eventTypes.SYNC_COMPLETE});c()}):(d.fireEvent(null,{eventType:b.eventTypes.SYNC_COMPLETE}),c())})):0<this.suspendClientFeeds.length?(this.fireEvent(null,{eventType:b.eventTypes.SYNC_START}),this.getDataForFeeds(this.suspendClientFeeds,function(){d.fireEvent(null,{eventType:b.eventTypes.SYNC_COMPLETE});c()})):c()};fjs.fdp.SyncManager.prototype.startSync=function(a){var b={},d=this;a?fjs.utils.Core.asyncForIn(a,function(a,f,g){d.getVersions(f,b,function(){g()})},
function(){d.transport.send({type:"synchronize",data:{versions:b}})}):Error("You must set feeds names array")};fjs.fdp.SyncManager.prototype.saveVersions=function(a,b,d){this.db&&(void 0!==d&&0>this.runtimefeeds.indexOf(a)&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.api.TabsSynchronizer).isMaster))&&this.db.insertOne("versions",{feedSource:a+"_"+b,feedName:a,source:b,version:d})};fjs.fdp.SyncManager.prototype.getVersions=function(a,b,d){var e=[],f=this;this.versions[a]?
setTimeout(function(){d(b[a]=f.versions[a])},0):this.db&&0>this.runtimefeeds.indexOf(a)?this.db.selectByIndex("versions",{feedName:a},function(a){e.push(a.source+"@"+a.version)},function(){f.versions[a]=b[a]=e.join(",");d(b[a])}):setTimeout(function(){d(f.versions[a]=b[a]="")},0)};fjs.fdp.SyncManager.prototype.lazySyncProcessItems=function(a,c,d){for(var e=[],f=0;f<a.length;f++){var g=a[f],h=g.xef001type||b.eventTypes.ENTRY_CHANGE,i=g.xpid=g.xpid?g.xpid:d?d+"_"+g.xef001id:g.xef001id;delete g.xef001type;
...
a,c);break;case b.syncTypes.FULL:this.fullSyncProcessItems(f,a,c);break;case b.syncTypes.KEEP:this.keepSyncProcessItems(f,a,c);break;default:fjs.utils.Console.error("Unknown sync type: "+d)}this.fireEvent(a,{eventType:b.eventTypes.SOURCE_COMPLETE,feed:a,sourceId:c,syncType:d});this.saveVersions(a,c,e)};fjs.fdp.SyncManager.prototype.onSync=function(a){var a=fjs.utils.JSON.parse(a),c=this;null!=this.syncTimeoutId&&(clearTimeout(this.syncTimeoutId),this.syncTimeoutId=null);this.syncs.push(a);if(1==this.syncs.length){this.fireEvent(null,
{eventType:b.eventTypes.SYNC_START});for(a=0;a<this.syncs.length;a++)for(var d in this.syncs[a])this.syncs[a].hasOwnProperty(d)&&this.processFeedData(d,this.syncs[a][d]);c.syncs=[];this.syncTimeoutId=setTimeout(function(){c.fireEvent(null,{eventType:b.eventTypes.SYNC_COMPLETE});c.syncTimeoutId=null},100)}else fjs.utils.Console.log("!!!!Double onSync!!",this.syncs[0],a)};fjs.fdp.SyncManager.prototype.sendAction=function(a,c,d){d.action=c;a={type:"action",data:{feedName:a,actionName:c,parameters:d}};
this.status!=b.states.READY?this.suspendActions.push(a):this.transport.send(a)};fjs.fdp.SyncManager.prototype.onClientSync=function(a,b){fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()&&(new fjs.api.TabsSynchronizer).isMaster?fjs.fdp.transport.LocalStorageTransport.masterSend("message",{type:"sync",data:a}):fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()&&!b&&fjs.fdp.transport.LocalStorageTransport.masterSend("clientSync",{type:"clientSync",data:a});if(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||
(new fjs.api.TabsSynchronizer).isMaster)if(1!=this.status)this.suspendClientSyncs.push(a);else this.onSync(a)};fjs.fdp.SyncManager.prototype.getAuthTicket=function(){var a=this;this.state==b.states.READY&&this.db?this.db.clear(function(){a.fireEvent("authError",{type:"authError",message:"Auth ticket wrong or expired"})}):a.fireEvent("authError",{type:"authError",message:"Auth ticket wrong or expired"})};fjs.fdp.SyncManager.prototype.addFeedListener=function(a,c,d){var e=this;0>this.feeds.indexOf(a)&&
this.feeds.push(a);this.superClass.addEventListener.apply(this,arguments);this.status!=b.states.READY?d?0>this.suspendClientFeeds.indexOf(a)&&this.suspendClientFeeds.push(a):(0>this.suspendFeeds.indexOf(a)&&this.suspendFeeds.push(a),0>this.syncFeeds.indexOf(a)&&this.syncFeeds.push(a)):0>this.syncFeeds.indexOf(a)?(d||(this.syncFeeds.push(a),0>this.versionsFeeds.indexOf(a)&&this.versionsFeeds.push(a),null!=this.versionsTimeoutId&&(clearTimeout(this.versionsTimeoutId),this.versionsTimeoutId=null),this.versionsTimeoutId=
setTimeout(function(){e.startSync(e.versionsFeeds);e.versionsFeeds=[];e.versionsTimeoutId=null},100)),this.fireEvent(a,{eventType:b.eventTypes.SYNC_START,feed:a},c),this.getFeedData(a,function(d){d.eventType==b.eventTypes.FEED_COMPLETE&&e.fireEvent(a,{eventType:b.eventTypes.SYNC_COMPLETE,feed:a},c);c(d)})):0<=this.syncFeeds.indexOf(a)&&this.states==b.READY&&(this.fireEvent(a,{eventType:b.eventTypes.SYNC_START,feed:a},c),this.getFeedData(a,function(d){d.eventType==b.eventTypes.FEED_COMPLETE&&e.fireEvent(a,
{eventType:b.eventTypes.SYNC_COMPLETE,feed:a},c);c(d)}))};fjs.fdp.SyncManager.prototype.removeFeedListener=function(a,b,d){var e;(e=0<=this.feeds.indexOf(a))&&this.feeds.splice(e,1);this.superClass.removeEventListener.apply(this,arguments);d||(-1<(e=this.syncFeeds.indexOf(a))&&this.syncFeeds.splice(e,1),this.listeners[a]&&0==this.listeners[a].length&&this.transport.send({type:"forget",data:{feedName:a}}))};fjs.fdp.SyncManager.prototype.getFeedData=function(a,c){var d=this;this.fireEvent(a,{eventType:b.eventTypes.FEED_START,
syncType:"F",feed:a},c);this.db?this.db.selectAll(a,function(e){d.fireEvent(a,{eventType:b.eventTypes.ENTRY_CHANGE,feed:a,xpid:e.xpid,entry:e},c)},function(){d.fireEvent(a,{eventType:b.eventTypes.FEED_COMPLETE,feed:a},c)}):this.fireEvent(a,{eventType:b.eventTypes.FEED_COMPLETE,feed:a},c)};fjs.fdp.SyncManager.prototype.fireEvent=function(a,b,d){if(d)d(b);else if(a)this.superClass.fireEvent.call(this,a,b);else for(var e in this.listeners)this.listeners.hasOwnProperty(e)&&-1<this.feeds.indexOf(e)&&this.superClass.fireEvent.call(this,
e,b)};fjs.fdp.SyncManager.prototype.loadNext=function(a,b,d){var e=fjs.utils.JSON.stringify(b),b=this.historyversions[a];b||(b={},this.historyversions[a]=b);if(b[e])this.processHistoryVersions(a,e,d,b[e],function(){});else{var f=this,g=[];this.fillHistoryVersions(a,e,d,g,function(){f.processHistoryVersions(a,e,d,g,function(){})})}};fjs.fdp.SyncManager.prototype.fillHistoryVersions=function(a,b,d,e,f){if(this.db){var g=this;this.db.selectByIndex("historyversions",{feedName:a,filter:b},function(a){e.push(a.source+
":"+a.version)},function(){0==e.length?g.fillEmptyHistoryVersionsFromFeed(a,e,f):f()})}};fjs.fdp.SyncManager.prototype.fillEmptyHistoryVersionsFromFeed=function(a,b,d){this.db&&this.db.selectByIndex("versions",{feedName:a},function(a){b.push(a.source+":0")},d)};fjs.fdp.SyncManager.prototype.processHistoryVersions=function(a,b,d,e,f){0==e.length?f&&f(!1):(this.historyversions[a][b]=e,b={"s.limit":d,"sh.filter":b,"sh.versions":e.join("@")},this.transport.send({type:"loadNext",data:{feedName:a,data:b}}),
f&&f(!0))};fjs.fdp.SyncManager.prototype.saveHistoryVersions=function(a,b,d,e){if(this.db&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||(new fjs.api.TabsSynchronizer).isMaster))if(this.db.insertOne("historyversions",{feedSourceFilter:a+"_"+b+"_filter",feedName:a,source:b,filter:d,version:e}),b=this.historyversions[a])delete b[d],b||delete this.historyversions[a]};fjs.fdp.SyncManager.prototype.saveEmptyHistoryVersions=function(a,b){if(this.db&&(!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()||
(new fjs.api.TabsSynchronizer).isMaster))this.db.insertOne("historyversions",{feedName:a,filter:b,version:"-1"}),delete this.historyversions[a]};fjs.fdp.SyncManager.prototype.logout=function(){this.node=this.ticket=null;this.getAuthTicket();this.status=b.states.NOT_INITIALIZED};fjs.fdp.SyncManager.prototype.SFLogin=function(a){var c=this;this.status==b.states.READY&&this.db?this.db.clear(function(){c.fireEvent(null,{eventType:b.eventTypes.CLEAR});for(var d in c.versions)c.versions.hasOwnProperty(d)&&
(c.versions[d]="");fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()&&(new fjs.api.TabsSynchronizer).isMaster&&fjs.fdp.transport.LocalStorageTransport.masterSend("message",{type:b.eventTypes.CLEAR});c.transport.send({type:"SFLogin",data:a})}):this.db?this.suspendLoginData=a:this.transport.send({type:"SFLogin",data:a})}})();namespace("fjs.fdp");
...
this.sm);case "sortings":return new fjs.fdp.model.ClientFeedProxyModel(["sortings"],this.sm);case "mycallsclient":return new fjs.fdp.model.MyCallsClientProxyModel(["mycalls","mycalldetails","mycallsclient"],this.sm);case "clientsettings":return new fjs.fdp.model.ClientFeedProxyModel(["clientsettings"],this.sm);case "clientcalllog":return new fjs.fdp.model.ClientFeedProxyModel(["clientcalllog"],this.sm);default:return new fjs.fdp.model.ProxyModel([b],this.sm)}};
fjs.fdp.DataManager.prototype.logout=function(){this.sm.logout()};fjs.fdp.DataManager.prototype.sendAction=function(b,a,c){var d=this.proxies[b];d?d.sendAction(b,a,c):fjs.utils.Console.warn("DataManager: sendAction: no proxy model for feed: "+b)};fjs.fdp.DataManager.prototype.loadNext=function(b,a,c){this.proxies[b]||fjs.utils.Console.error("You don't listen this feed");this.sm.loadNext(b,a,c)};
fjs.fdp.DataManager.prototype.fireEvent=function(b,a){var c=this.listeners[b],d;a.eventType=b;if(c)for(d=0;d<c.length;d++)c[d](a);if(c=this.listeners[""])for(d=0;d<c.length;d++)c[d](a)};fjs.fdp.DataManager.prototype.SFLogin=function(b){this.sm.SFLogin(b)};
