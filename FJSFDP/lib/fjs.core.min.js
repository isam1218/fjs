...
(function(){namespace("fjs.ajax");fjs.ajax.IFrameRequest=function(a){this.id=a;this.status=0;this.responseText="";this.headers=this.postData=this.url=this.method=null};var a=fjs.ajax.IFrameAjax=function(b){if(this.constructor.__instance)return this.constructor.__instance;this.constructor.__instance=this;var c=this;this.host=b;this.status=a.states.NOT_INITIALIZED;this.suspendRequestes=[];this.listeners={};this.requests={};this.crdmnFrame=document.getElementById("crdmnFrame");this.compatibility=!0;
this.crdmnFrame?c.status=a.states.READY:(this.status=a.states.INITIALIZATION,this.crdmnFrame=document.createElement("iframe"),this.crdmnFrame.name=this.crdmnFrame.id="crdmnFrame",this.crdmnFrame.src=this.host,this.crdmnFrame.style.width=0,this.crdmnFrame.style.height=0,this.crdmnFrame.onload=function(){c.status=a.states.READY;for(var b=0;b<c.suspendRequestes.length;b++)c.send(null,null,null,null,null,c.suspendRequestes[b])},document.body.appendChild(this.crdmnFrame),window.addEventListener?window.addEventListener("message",
function(a){c.onResponse(a)},!1):window.attachEvent&&window.attachEvent("onmessage",function(a){c.onResponse(a)}))};fjs.ajax.IFrameAjax.states={NOT_INITIALIZED:-1,INITIALIZATION:0,READY:1};fjs.ajax.IFrameAjax.prototype.getParamData=function(a){var c=[],d;for(d in a)a.hasOwnProperty(d)&&c.push(d+"="+a[d]);return c.join("&")};fjs.ajax.IFrameAjax.prototype.send=function(b,c,d,f,g,e){this.compatibility?(e||(e=new fjs.ajax.IFrameRequest((new fjs.utils.Increment).get("IFrameAjaxRequest")),e.method=b,e.data=
this.getParamData(f),e.url=c,(b=d.Authorization)&&(b=b.replace("auth=","")),e.authTicket=b,e.node=d.node,this.listeners[e.id]=g),this.status!=a.states.READY)?this.suspendRequestes.push(e):(this.crdmnFrame.contentWindow.postMessage(JSON.stringify(e),"*"),this.requests[e.id]=e):(e||(e=new fjs.ajax.IFrameRequest((new fjs.utils.Increment).get("IFrameAjaxRequest")),e.method=b,e.postData=f,e.url=c,e.headers=d,this.listeners[e.id]=g),this.status!=a.states.READY)?this.suspendRequestes.push(e):(this.crdmnFrame.contentWindow.postMessage(JSON.stringify({type:"send",
data:e}),"*"),this.requests[e.id]=e);return e};fjs.ajax.IFrameAjax.prototype.abort=function(a){this.compatibility||this.crdmnFrame.contentWindow.postMessage(fjs.utils.JSON.stringify({type:"abort",data:a}),"*");a.aborted=!0;a.status=0;if(this.listeners[a.id])this.listeners[a.id](a,"",!1);delete this.listeners[a.id];delete this.requests[a.id]};fjs.ajax.IFrameAjax.prototype.onResponse=function(a){if(fjs.utils.JSON.check(a.data))if(a=fjs.utils.JSON.parse(a.data),this.compatibility){if(a){var c=a.id,d=
this.requests[c];if(d){d.status=a.status;d.responseText=a.response;var f=this.listeners[c];f&&(a.error?f(d,d.responseText,!1):f(d,d.responseText,!0),delete this.listeners[c])}delete this.requests[c]}}else if(a&&"IFrameAjaxRequest"==a.type){a=a.data;c=a.id;if(d=this.requests[c])if(d.status=a.status,d.responseText=a.responseText,f=this.listeners[c])a.error?f(d,d.responseText,!1):f(d,d.responseText,!0),delete this.listeners[c];delete this.requests[c]}}})();
(function(){namespace("fjs.ajax");fjs.ajax.XDRAjax=function(){if(this.constructor.__instance)return this.constructor.__instance;this.constructor.__instance=this};fjs.ajax.XDRAjax.prototype.send=function(a,b,c,d,f){var g=new XDomainRequest;g._onerror=g.onerror=function(a){a=a||window.event;a=a.target||g;f(a,a.responseText,!1)};g.onload=function(a){a=a||window.event;a=a.target||g;a.status=200;f(a,a.responseText,!0)};for(var e in c)c.hasOwnProperty(e)&&(b+=(/\?/.test(b)?"&":"?")+e+"="+encodeURIComponent(c[e]));
g.open(a,b);g.send(this.getParamData(d));return g};fjs.ajax.XDRAjax.prototype.getParamData=function(a){var b=[],c;for(c in a)a.hasOwnProperty(c)&&b.push(c+"="+a[c]);return b.join("&")};fjs.ajax.XDRAjax.prototype.abort=function(a){a.aborted=!0;a.status=0;a.abort();a._onerror({target:a})}})();
