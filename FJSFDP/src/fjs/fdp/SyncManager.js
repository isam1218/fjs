(function(){
    ...
    fjs.fdp.SyncManager = function(config) {
        ...
        this.dbFactory = new fjs.db.DBFactory(config.DB);
        ...
        /**
         *
         * @type {fjs.tabs.TabsSynchronizer}
         * @private
         */
    ...
    fjs.core.inherits(fjs.fdp.SyncManager, fjs.model.EventsSource);
    ...
    fjs.fdp.SyncManager.prototype.init = function(ticket, node, callback) {
        ...
        if(fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization()) {
            this.tabsSyncronizer = new fjs.tabs.TabsSynchronizer();
    ...
    fjs.fdp.SyncManager.prototype.onStorage = function(e) {
        if(e.key.indexOf('lsp_')>-1) {
            ...
            if(new fjs.tabs.TabsSynchronizer().isMaster) {
    ...
    fjs.fdp.SyncManager.prototype.addTransportEvents = function() {
        ...
        this.transport.addEventListener('message', function(e){
            if(fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization() && new fjs.tabs.TabsSynchronizer().isMaster) {
    ...
    fjs.fdp.SyncManager.prototype.initDataBase = function(callback) {
        var context = this;
        this.dbFactory.getDB(function(db){
            context.db = db;
            context.finishInitialization(callback);
            if(context.db && fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization() && context.tabsSyncronizer.isMaster) {
    ...
    fjs.fdp.SyncManager.prototype.getDataForFeeds = function (feeds, callback) {
        var context = this;
        fjs.core.asyncForIn(feeds, function(key, value, next){
    ...
    fjs.fdp.SyncManager.prototype.saveVersions = function(feedName, source, version) {
        if(this.db && version !== undefined && this.runtimefeeds.indexOf(feedName) < 0 && (!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization() || new fjs.tabs.TabsSynchronizer().isMaster)) {
    ...
    fjs.fdp.SyncManager.prototype.lazySyncProcessItems = function(items, feedName, sourceId) {
        ...
        if(this.db && entriesForSave.length>0 && (!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization() || new fjs.tabs.TabsSynchronizer().isMaster)) {
    ...
    fjs.fdp.SyncManager.prototype.fullSyncProcessItems = function(items, feedName, sourceId) {
        ...
        if(this.db && (!fjs.fdp.transport.TransportFactory.useLocalStorageSyncronization() || new fjs.tabs.TabsSynchronizer().isMaster)) {
    ...
    fjs.fdp.SyncManager.prototype.onSync = function (data) {
        ...
        if(this.syncs.length==1) {
            ...
            for(var i=0; i <this.syncs.length; i++) {
                var keys = Object.keys(this.syncs[i]);
     ...
     * @param {boolean=} isClientFeed True if is client feed
     */
    fjs.fdp.SyncManager.prototype.addFeedListener = function(feedName, listener, isClientFeed) {
        ...
        sm.super_.prototype.addEventListener.apply(this, arguments);
    ...
    fjs.fdp.SyncManager.prototype.removeFeedListener = function(feedName, listener, isClient) {
        ...
        sm.super_.prototype.removeEventListener.apply(this, arguments);
        if(!isClient) {
    ...
    fjs.fdp.SyncManager.prototype.fireEvent = function(feedName, data, listener) {
        if(listener) {
            ...
            sm.super_.prototype.fireEvent.call(this, feedName, data);
        }
        else {
            var keys = Object.keys(this.listeners);
            for(var i=0; i<keys.length; i++) {
              var _feedName = keys[i];
              if(this.feeds.indexOf(_feedName)>-1) {
                sm.super_.prototype.fireEvent.call(this, _feedName, data);
              }
            }
        }
    };
     ...
     */
    fjs.fdp.SyncManager.prototype.logout = function(handler) {
        var context = this;
        this.ticket = null;
        this.node = null;
        if (this.status == sm.states.READY && this.db) {
            this.db.clear(function () {
                context.fireEvent(null, {eventType: sm.eventTypes.CLEAR});
                var keys = Object.keys(context.versions);
                for(var i=0; i<keys.length; i++) {
                  var key = keys[i];
                ...
                context.status = sm.states.NOT_INITIALIZED;
                handler();
            });
        }
        else {
