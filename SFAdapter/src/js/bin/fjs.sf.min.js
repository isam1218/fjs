...
(f=f[this.phone])&&(h=this.findCallLogTargetById(f.id))?"who"==this.getRelatedItemType(h)?(this.mycallsclient_callLog.whoId=f.id,"Lead"==h.object&&(this.mycallsclient_callLog.whatId=null)):(h=this.getWho(!0),this.mycallsclient_callLog.whoId=h&&h._id,this.mycallsclient_callLog.whatId=f.id):(h=this.findCallLogTargetById(this.mycallsclient_callLog.whoId),this.findCallLogTargetById(this.mycallsclient_callLog.whatId),h||(h=this.getWho(),this.mycallsclient_callLog.whoId=h&&h._id));f&&b.deletePhone(this.phone);
...
namespace("fjs.model");fjs.model.ClientCallLogFeedModel=function(a){fjs.model.FeedModel.call(this,"clientcalllog",a)};fjs.model.ClientCallLogFeedModel.extend(fjs.model.FeedModel);fjs.model.ClientCallLogFeedModel.prototype.onEntryChange=function(a){var b=this.items[a.xpid];b?b.fill(a.entry):(b=this.items[a.xpid]=new fjs.model.CallLogEntryModel(a.entry),this.order.push(b));this.prepareEntry(b);this.fireEvent(fjs.model.FeedModel.EVENT_TYPE_PUSH,b)};
fjs.model.CallLogEntryModel=function(a){fjs.model.EntryModel.call(this,a)};fjs.model.CallLogEntryModel.extend(fjs.model.EntryModel);fjs.model.CallLogEntryModel.prototype.fill=function(a,b){b=b||this;if(a)for(var c in a)if(a.hasOwnProperty(c)){var d=a[c];if("object"!=typeof d||null==d){if("note"!=c||!this._blockChangeNote)b[c]=d}else Array.isArray(d)?b[c]=[]:b[c]||(b[c]={}),this.fill(d,b[c])}};namespace("fjs.model.filter");
fjs.model.filter.What=function(){return function(a){return a.filter(function(a){return"Contact"!=a.object&&"Lead"!=a.object})}};namespace("fjs.model.filter");fjs.model.filter.Who=function(){return function(a){return a.filter(function(a){return"Contact"==a.object||"Lead"==a.object})}};namespace("fjs.model.filter");
