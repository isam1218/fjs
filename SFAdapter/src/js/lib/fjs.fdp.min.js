...
fjs.db.WebSQLProvider.prototype.open=function(b,a,c){var d=this;try{var e=this.db=self.openDatabase(b,"",b,5232645)}catch(f){console.error(f);c(null);return}if(""===e.version){for(var g in d.tables)d.tables.hasOwnProperty(g)&&(b=d.tables[g],d.createTable(g,b.key,b.indexes),console.log(g+" table created"));c(this)}else e.version!==a?e.changeVersion(e.version,a,function(){d.clear(function(){for(var a in d.tables)if(d.tables.hasOwnProperty(a)){var b=d.tables[a];d.createTable(a,b.key,b.indexes)}c(this)})}):
c(this)};fjs.db.WebSQLProvider.prototype.createTable=function(b,a,c){this.db.transaction(function(d){var e;e="CREATE TABLE IF NOT EXISTS "+(b+"("+a+" TEXT PRIMARY KEY");e+=0<c.length?", "+c.join(" TEXT, ")+" TEXT":"";d.executeSql(e+", data TEXT)");if(c)for(e=0;e<c.length;e++)d.executeSql("CREATE INDEX "+b+"_"+c[e]+"_idx ON "+b+" ("+c[e]+")")})};
...
fjs.db.WebSQLProvider.prototype.insertArray=function(b,a,c){var d=this.tables[b],e=a.length;this.db.transaction(function(f){for(var g="INSERT OR REPLACE INTO "+b+"("+d.key+", "+(d.indexes?d.indexes.join(", "):"")+", data) VALUES ('",h=0;h<a.length;h++){var j=a[h],i=g+j[d.key]+"', '";if(d.indexes)for(var k=0;k<d.indexes.length;k++)i+=j[d.indexes[k]]+"', '";i+=fjs.utils.JSON.stringify(j)+"')";f.executeSql(i,[],function(){e--;c&&0==e&&c()},function(a){throw Error(a);})}})};
...
fjs.db.WebSQLProvider.prototype.selectAll=function(b,a,c){this.db.transaction(function(d){d.executeSql("SELECT data FROM "+b,[],function(b,d){var g=[];if(d.rows){for(var h=0;h<d.rows.length;h++){var j=d.rows.item(h).data,j=JSON.parse(j);g.push(j);a&&a(j)}c&&c(g)}},function(a){throw Error(a);})})};
...
fjs.db.WebSQLProvider.prototype.clear=function(b){this.db.transaction(function(a){a.executeSql("SELECT 'drop table ' || name || ';' FROM sqlite_master WHERE type = 'table' AND name NOT GLOB '_*'",[],b,function(a){throw Error(a);})})};
...
fjs.db.LocalStorageDbProvider.prototype.createIndexes=function(b,a){var c=this.getLSTableName(b);this.dbData[c]||(this.dbData[c]={});if(this.tables[b].indexes&&0<this.tables[b].indexes.length){var d=this.indexes[c];d||(d=this.indexes[c]={});for(c=0;c<this.tables[b].indexes.length;c++){var e=this.tables[b].indexes[c],f;fjs.utils.Array.isArray(e)&&(f=e,f.sort(),e=f.join("_"));var g=d[e];g||(g=d[e]={});for(var h in a)if(a.hasOwnProperty(h)){var j,i;if(f){j=[];for(i=0;i<f.length;i++)j.push(a[h][f[i]]);
i=j.join("_")}else i=a[h][e];(j=g[i])||(j=g[i]=[]);i=a[h][this.tables[b].key];0>j.indexOf(i)&&j.push(i)}}}};
fjs.db.LocalStorageDbProvider.prototype.open=function(b,a,c){var d,e=this;if(this.dbInfo=fjs.utils.JSON.parse(self.localStorage.getItem("DB_"+b)))if(a>this.dbInfo.version){for(d in this.dbInfo.tables)this.dbInfo.tables.hasOwnProperty(d)&&self.localStorage.removeItem(d);this.dbInfo.tables=null;this.dbInfo.version=a;this.createTables()}else for(b=0;b<this.dbInfo.tables.length;b++)d=this.dbInfo.tables[b],a=this.dbData[d]=fjs.utils.JSON.parse(self.localStorage.getItem(d))||{},this.createIndexes(this.getRealTableName(d),
a);else this.dbInfo={name:b,version:a},this.createTables(),self.localStorage.setItem("DB_"+b,fjs.utils.JSON.stringify(this.dbInfo));setTimeout(function(){c(e)},0)};fjs.db.LocalStorageDbProvider.check=function(){return!!self.localStorage};fjs.db.LocalStorageDbProvider.prototype.createTable=function(){};fjs.db.LocalStorageDbProvider.prototype.createTables=function(){if(this.tables){this.dbInfo.tables=[];for(var b in this.tables)this.tables.hasOwnProperty(b)&&this.dbInfo.tables.push(this.getLSTableName(b))}};
fjs.db.LocalStorageDbProvider.prototype.getLSTableName=function(b){return"DB_"+this.dbInfo.name+"_"+b};fjs.db.LocalStorageDbProvider.prototype.getRealTableName=function(b){return b.replace("DB_"+this.dbInfo.name+"_","")};fjs.db.LocalStorageDbProvider.prototype.declareTable=function(b,a,c){this.tables[b]={name:b,key:a,indexes:c}};
fjs.db.LocalStorageDbProvider.prototype.insertOne=function(b,a,c){var d=this.getLSTableName(b);this.createIndexes(b,[a]);this.dbData[d][a[this.tables[b].key]]=a;self.localStorage.setItem(d,fjs.utils.JSON.stringify(this.dbData[d]));setTimeout(c,0)};
fjs.db.LocalStorageDbProvider.prototype.insertArray=function(b,a,c){var d=this.getLSTableName(b);this.createIndexes(b,a);for(var e=0;e<a.length;e++){var f=a[e];this.dbData[d][f[this.tables[b].key]]=f}self.localStorage.setItem(d,fjs.utils.JSON.stringify(this.dbData[d]));setTimeout(c,0)};
fjs.db.LocalStorageDbProvider.prototype.deleteByKey=function(b,a,c){b=this.getLSTableName(b);null!=a&&this.dbData[b]?delete this.dbData[b][a]:(this.dbData[b]={},this.indexes[b]={});self.localStorage.setItem(b,fjs.utils.JSON.stringify(this.dbData[b]));setTimeout(c,0)};
fjs.db.LocalStorageDbProvider.prototype.selectAll=function(b,a,c){var b=this.getLSTableName(b),d=[],e=this.dbData[b],b=0,f;for(f in e)e.hasOwnProperty(f)&&(b++,function(b,f){setTimeout(function(){var j=e[b];d.push(j);a(j);--f;0===f&&setTimeout(function(){c(d)},0)},0)}(f,b));0===b&&setTimeout(function(){c(d)},0)};
fjs.db.LocalStorageDbProvider.prototype.selectByIndex=function(b,a,c,d){var e=[],f=[],g=[],h=this.getLSTableName(b),j=[],i,k,l=this,m=0;if(a){for(var n in a)a.hasOwnProperty(n)&&f.push(n);if(1<f.length){f.sort();i=f.join("_");for(b=0;b<f.length;b++)g.push(a[f[b]]);k=g.join("_")}else 1==f.length&&(i=f[0],k=a[i]);if((j=this.indexes[h]&&this.indexes[h][i]&&this.indexes[h][i][k])&&0<j.length)for(a=0;a<j.length;a++)m++,function(a,b){setTimeout(function(){var f=l.dbData[h][j[a]];e.push(f);c(f);--b;0==b&&
setTimeout(function(){d(e)},0)},0)}(a,m);else setTimeout(function(){d(e)},0)}else this.selectAll(b,c,d)};fjs.db.LocalStorageDbProvider.prototype.selectByKey=function(b,a,c){b=this.getLSTableName(b);setTimeout(c(this.dbData[b][a]),0)};fjs.db.LocalStorageDbProvider.prototype.clear=function(b){for(var a in this.tables)if(this.tables.hasOwnProperty(a)){var c=this.getLSTableName(a);self.localStorage.removeItem(c);delete this.dbData[c];delete this.indexes[c]}setTimeout(b,0)};
fjs.db.LocalStorageDbProvider.prototype.deleteByIndex=function(b,a,c){var d=[],e=[],f=[],b=this.getLSTableName(b),g=[],h,j;if(null==a){for(var i in this.dbData[b])this.dbData[b].hasOwnProperty(i)&&(d.push(this.dbData[b][i]),delete this.dbData[b][i]);this.indexes[b]={};self.localStorage.setItem(b,fjs.utils.JSON.stringify(this.dbData[b]))}else{for(var k in a)a.hasOwnProperty(k)&&e.push(k);if(1<e.length){e.sort();h=e.join("_");for(g=0;g<e.length;g++)f.push(a[e[g]]);j=f.join("_")}else 1==e.length&&(h=
e[0],j=a[h]);if(g=this.indexes[b]&&this.indexes[b][h]&&this.indexes[b][h][j])for(a=0;a<g.length;a++)d.push(this.dbData[b][g[a]]),delete this.dbData[b][g[a]]}setTimeout(function(){c(d)},0)};namespace("fjs.db");fjs.db.DBFactory=function(b){this.config=b;this._dbRegister={indexedDB:fjs.db.IndexedDBProvider,webSQL:fjs.db.WebSQLProvider,localStorage:fjs.db.LocalStorageDbProvider}};fjs.db.DBFactory.prototype.getDB=function(){for(var b=this.config.DB.dbProviders,a=0;a<b.length;a++)if(this._dbRegister[b[a]].check())return new this._dbRegister[b[a]]};
...
fjs.fdp.model.ProxyModel.prototype.addListener=function(b){if(0>this.listeners.indexOf(b)){this.listeners.push(b);var a=this.createFullChange();a&&b({feed:this.feedName,changes:a});this._attached||(this.attach(),this._attached=!0)}else console.warn("Trying to add duplicated listener")};fjs.fdp.model.ProxyModel.prototype.removeListener=function(b){b=this.listeners.indexOf(b);-1<b?(this.listeners.splice(b,1),0==this.listeners.length&&(this.detach(),this._attached=!1)):console.warn("Trying to remove unexisted listener")};
...
fjs.fdp.model.ProxyModel.prototype.createFullChange=function(){var b={},a=0,c;for(c in this.items)this.items.hasOwnProperty(c)&&(b[c]={xpid:c,entry:this.items[c],type:"change"},a++);return 0<a?b:null};fjs.fdp.model.ProxyModel.prototype.fillChange=function(b,a,c){this.collectFields(c,a);b=this.createChange(b);if("delete"!=b.type){b.type="change";for(var d in a)a.hasOwnProperty(d)&&this.fieldPass(c,d,!0)&&(b.entry[d]=a[d])}else console.error("Change for deleted entry");return b};
fjs.fdp.model.ProxyModel.prototype.fillDeletion=function(b,a){var c=this.createChange(b);if(a==this.feedName)c.type="delete",c.entry=null;else{if("delete"==c.type)return c;c.type="change";var d=this.feedFields[a];if(d){if(this.items[b])for(var e in d)d.hasOwnProperty(e)&&e in this.items[b]&&(delete this.items[b][e],c.entry[e]=d[e]);else console.error("Trying to remove unexisted element:",b);fjs.utils.JSON.isEmpty(c.entry)&&(delete this.changes[b],c=null,fjs.utils.JSON.isEmpty(this.changes)&&(this.changes=
null))}}return c};fjs.fdp.model.ProxyModel.prototype.fieldPass=function(b,a,c){return b!=this.feedName?a!=b+"_xef001id"&&a!=b+"_xef001iver"&&a!=b+"_xef001type"&&a!=b+"_xpid"&&a!=b+"_source"&&("xpid"!=a||c):("xpid"!=a||c)&&"xef001id"!=a&&"xef001iver"!=a&&"xef001type"!=a&&"source"!=a};fjs.fdp.model.ProxyModel.prototype.collectFields=function(b,a){if(b!=this.feedName&&!this.feedFields[b]){this.feedFields[b]={};for(var c in a)a.hasOwnProperty(c)&&this.fieldPass(b,c,!1)&&(this.feedFields[b][c]=null)}};
fjs.fdp.model.ProxyModel.prototype.prepareEntry=function(b,a,c){var d={},e,a=a!=this.feedName?a+"_":"";for(e in b)b.hasOwnProperty(e)&&(d[a+e]=b[e]);d.xpid=c;return d};fjs.fdp.model.ProxyModel.prototype.onEntryChange=function(b){var a=this.items[b.xpid],c=this.prepareEntry(b.entry,b.feed,b.xpid);a?(a=a.fill(c))&&this.fillChange(b.xpid,a,b.feed):"delete"!=this.fillChange(b.xpid,c,b.feed).type&&(this.items[b.xpid]=new fjs.fdp.model.EntryModel(c));this.keepEntries[b.xpid]=b};
fjs.fdp.model.ProxyModel.prototype.onEntryDeletion=function(b){b.feed==this.feedName&&delete this.items[b.xpid];this.fillDeletion(b.xpid,b.feed)};fjs.fdp.model.ProxyModel.prototype.onEntryKeep=function(b){this.keepEntries[b.xpid]=b};
fjs.fdp.model.ProxyModel.prototype.onSourceComplete=function(b){if(b.syncType==fjs.fdp.SyncManager.syncTypes.FULL||b.syncType==fjs.fdp.SyncManager.syncTypes.KEEP)for(var a in this.items)if(this.items.hasOwnProperty(a)&&b.sourceId==this.items[a].source&&!this.keepEntries[a])this.onEntryDeletion({eventType:fjs.fdp.SyncManager.eventTypes.ENTRY_DELETION,feed:b.feed,xpid:a,entry:null});this.keepEntries={}};
...
fjs.fdp.model.ClientFeedProxyModel.prototype.addListener=function(b){if(0>this.listeners.indexOf(b)){this.listeners.push(b);var a=this.createFullChange();a&&b({feed:this.clientFeedName,changes:a});this._attached||(this.attach(),this._attached=!0)}else console.warn("Trying to add duplicated listener")};
...
this;this.sendRequest(this.url+this.SF_LOGIN_PATH,b,function(b,d,e){if(e){var f=fjs.utils.JSON.parse(d),d=f.Auth,f=f.node;d&&f?(a.fireEvent("message",{type:"node",data:{nodeId:a.node=f}}),a.fireEvent("message",{type:"ticket",data:{ticket:a.ticket=d}})):a.fireEvent("error",{type:"authError",message:"Can't get ticket or node"})}else(403==b.status||401==b.status)&&a.fireEvent("error",{type:"authError",message:"Wrong auth data"});a.handleRequestErrors(b,e)})};fjs.fdp.transport.AJAXTransport.prototype.loadNext=
function(b){var a=b.data,c=this;this.sendRequest(this.url+"/v1/history/"+a.feedName,a.data,function(b,e,f){if(f){var e=fjs.utils.JSON.parse(e),g={};g[a.feedName]=e;for(var h in e)e.hasOwnProperty(h)&&(e[h].filter=a.data["sh.filter"]);c.fireEvent("message",{type:"sync",data:g})}c.handleRequestErrors(b,f)})};fjs.fdp.transport.AJAXTransport.prototype._forgetFeed=function(b){0>this._forgetFeeds.indexOf(b)&&this._forgetFeeds.push(b)};fjs.fdp.transport.AJAXTransport.prototype.parseClientRegistryResponse=
function(b){for(var b=b.split("\n"),a={},c=0;c<b.length;c++)if(-1<b[c].indexOf("=")){var d=b[c].split("="),e=d[0].trim(),d=d[1].trim();e&&d&&(a[e]=d)}return a};fjs.fdp.transport.AJAXTransport.prototype.requestClientRegistry=function(b){var a=this;this._currentRequest=this.sendRequest(this.url+this.CLIENT_REGISRY_PATH,{},function(c,d,e){if(e){this._clientRegistryFailedCount=0;var f=a.parseClientRegistryResponse(d).node;f&&(a.fireEvent("message",{type:"node",data:{nodeId:a.node=f}}),b(d))}else 403==
c.status?(b(!1),a.fireEvent("error",{type:"authError",message:"Auth ticket wrong or expired"})):10>this._clientRegistryFailedCount?(this._clientRegistryFailedCount++,a.requestClientRegistry(b)):b(!1);a.handleRequestErrors(c,e)})};fjs.fdp.transport.AJAXTransport.prototype.requestVersionscache=function(){var b=this;this._currentRequest=this._currentVersioncache=this.sendRequest(this.url+this.VERSIONSCACHE_PATH,null,function(a,c,d){d?(b._clientRegistryFailedCount=0,(c=b.parseVersionsResponse(c))?b.syncRequest(c):
b.requestVersionscache()):401==a.status||403==a.status?b.requestClientRegistry(function(a){a&&b.requestVersionscache()}):a.aborted||(5>b._clientRegistryFailedCount?(b.requestVersionscache(),b._clientRegistryFailedCount++):5<=b._clientRegistryFailedCount&&setTimeout(function(){b.requestVersionscache()},5E3));b.handleRequestErrors(a,d)})};fjs.fdp.transport.AJAXTransport.prototype.syncRequest=function(b){var a=this;this._currentRequest=this.sendRequest(this.url+this.SYNC_PATH,b,function(b,d,e){e&&(d=
fjs.utils.JSON.parse(d),a.fireEvent("message",{type:"sync",data:d}),a.requestVersionscache());a.handleRequestErrors(b,e)})};fjs.fdp.transport.AJAXTransport.prototype.parseVersionsResponse=function(b){var b=b.split(";"),a={},c=0,d;if(b)for(var e=2;e<b.length-1;e++)b[e]&&(0>(d=this._forgetFeeds.indexOf(b[e]))?a[b[e]]="":this._forgetFeeds.splice(d,1),c++);return c?a:null};fjs.fdp.transport.AJAXTransport.prototype.requestVersions=function(b){var a=this;null!=this._currentVersioncache&&this.ajax.abort(this._currentVersioncache);
if(!this.node)return this.requestClientRegistry(function(c){c&&a.requestVersions(b)});this._currentRequest=this.sendRequest(this.url+this.VERSIONS_PATH,b,function(c,d,e){e?(d=a.parseVersionsResponse(d))?a.syncRequest(d):a.requestVersionscache():(401==c.status||403==c.status)&&a.requestClientRegistry(function(c){c&&a.requestVersions(b)});a.handleRequestErrors(c,e)})};fjs.fdp.transport.AJAXTransport.prototype.sendAction=function(b,a,c){var d=this,a={action:a},e;for(e in c)c.hasOwnProperty(e)&&(a["a."+
e]=c[e]);this.sendRequest(this.url+"/v1/"+b,a,function(a,b,c){d.handleRequestErrors(a,c)})};fjs.fdp.transport.AJAXTransport.prototype.close=function(){this.closed=!0;this._currentRequest&&this.ajax.abort(this._currentRequest)}})();
...
(function(){namespace("fjs.fdp");fjs.fdp.transport.XDRTransport=function(b,a,c,d){fjs.fdp.transport.AJAXTransport.call(this,b,a,c,d);this.ajax=new fjs.ajax.XDRAjax;this.iframeAjax=new fjs.ajax.IFrameAjax};fjs.fdp.transport.XDRTransport.extend(fjs.fdp.transport.AJAXTransport);fjs.fdp.transport.XDRTransport.prototype.sendRequest=function(b,a,c){var d=this;if(this.closed)return null;a.t=this.type;a.alt="j";var e={Authorization:"auth="+this.ticket,node:this.node};return this.ajax.send("post",b,e,a,function(f,
g,h){h?c.apply(this,arguments):d.iframeAjax.send("post",b,e,a,c)}).url=b}})();
...
(function(){fjs.fdp.transport.TransportFactory=function(){};fjs.fdp.transport.TransportFactory.getTransport=function(b,a,c,d){var e;if(fjs.fdp.TabsSynchronizer.useLocalStorageSyncronization())e=(new fjs.fdp.TabsSynchronizer).isMaster;else return new fjs.fdp.transport.XHRTransport(b,a,c,d);if(null!=e)if(e)this._getBrowserSpecifiedTransport(b,a,c,d);else return new fjs.fdp.transport.LocalStorageTransport;return this._getBrowserSpecifiedTransport(b,a,c,d)};fjs.fdp.transport.TransportFactory._getBrowserSpecifiedTransport=
function(b,a,c,d){return fjs.utils.Browser.isIE()&&10>fjs.utils.Browser.getIEVersion()?new fjs.fdp.transport.XDRTransport(b,a,c,d):new fjs.fdp.transport.XHRTransport(b,a,c,d)}})();
(function(){fjs.fdp.TabsSynchronizer=function(){if(this.constructor.__instance)return this.constructor.__instance;this.constructor.__instance=this;var b=this;fjs.EventsSource.call(this);this.CHANGE_TAB_TIMEOUT=2E3;this.MASTER_ACTIVITY_TIMEOUT=500;this.TABS_SYNCRONIZE_KEY="tabs_sync_maintab";this.tabId=Date.now()+"_"+fjs.utils.GUID.create();this.timeoutId=null;this.isMaster=!1;this._runMaster=function(){b._masterIteration()};this._masterIteration=function(){localStorage[b.TABS_SYNCRONIZE_KEY]=b.tabId+
"|"+Date.now();b.isMaster||b.fireEvent("master_changed",b.isMaster=!0);clearTimeout(b.timeoutId);b.timeoutId=setTimeout(b._masterIteration,b.MASTER_ACTIVITY_TIMEOUT)};self.addEventListener("storage",function(a){a.key==b.TABS_SYNCRONIZE_KEY&&(a=a.newValue.split("|"),a[0]!=b.tabId&&b.tabId>a[0]&&(b.isMaster&&b.fireEvent("master_changed",b.isMaster=!1),clearTimeout(b.timeoutId),b.timeoutId=setTimeout(b._runMaster,b.CHANGE_TAB_TIMEOUT)))},!1);var a=localStorage[this.TABS_SYNCRONIZE_KEY];!a||Date.now()-
parseInt(a.split("|")[1])>this.CHANGE_TAB_TIMEOUT?this._runMaster():this.timeoutId=setTimeout(this._runMaster,this.CHANGE_TAB_TIMEOUT)};fjs.fdp.TabsSynchronizer.extend(fjs.EventsSource);fjs.fdp.TabsSynchronizer.useLocalStorageSyncronization=function(){return"undefined"!==typeof window&&void 0!==window.document||self&&self.web_worker}})();
...
function(a,f,g){d.getVersions(f,b,function(){g()})},function(){d.transport.send({type:"synchronize",data:{versions:b}})}):Error("You must set feeds names array")};fjs.fdp.SyncManager.prototype.saveVersions=function(a,b,d){this.db&&(void 0!==d&&(!fjs.fdp.TabsSynchronizer.useLocalStorageSyncronization()||(new fjs.fdp.TabsSynchronizer).isMaster))&&this.db.insertOne("versions",{feedSource:a+"_"+b,feedName:a,source:b,version:d})};fjs.fdp.SyncManager.prototype.getVersions=function(a,b,d){var e=[],f=this;
this.versions[a]?setTimeout(function(){d(b[a]=f.versions[a])},0):this.db?this.db.selectByIndex("versions",{feedName:a},function(a){e.push(a.source+"@"+a.version)},function(){f.versions[a]=b[a]=e.join(",");d(b[a])}):setTimeout(function(){d(f.versions[a]=b[a]="")},0)};fjs.fdp.SyncManager.prototype.lazySyncProcessItems=function(a,c,d){for(var e=[],f=0;f<a.length;f++){var g=a[f],h=g.xef001type||b.eventTypes.ENTRY_CHANGE,j=g.xpid=g.xpid?g.xpid:d?d+"_"+g.xef001id:g.xef001id;delete g.xef001type;g.source=
d;g={eventType:h,feed:c,xpid:j,entry:g};if(this.db)switch(h){case b.eventTypes.ENTRY_CHANGE:e.push(g.entry);break;case b.eventTypes.ENTRY_DELETION:(!fjs.fdp.TabsSynchronizer.useLocalStorageSyncronization()||(new fjs.fdp.TabsSynchronizer).isMaster)&&this.db.deleteByKey(c,g.xpid,null);break;default:console.error("Incorrect item change type: "+h+" for Lazy sync")}this.fireEvent(c,g)}this.db&&(0<e.length&&(!fjs.fdp.TabsSynchronizer.useLocalStorageSyncronization()||(new fjs.fdp.TabsSynchronizer).isMaster))&&
this.db.insertArray(c,e,null)};fjs.fdp.SyncManager.prototype.fullSyncProcessItems=function(a,c,d){for(var e=this,f=[],g=0;g<a.length;g++){var h=a[g],j=h.xef001type||b.eventTypes.ENTRY_CHANGE;h.source=d;if(j===b.eventTypes.ENTRY_CHANGE){var i=h.xpid=h.xpid?h.xpid:d?d+"_"+h.xef001id:h.xef001id;delete h.xef001type;j={eventType:j,feed:c,xpid:i,entry:h};f.push(h);this.fireEvent(c,j)}else console.error("Incorrect item change type: "+j+" for Full sync")}this.db&&(!fjs.fdp.TabsSynchronizer.useLocalStorageSyncronization()||
(new fjs.fdp.TabsSynchronizer).isMaster)&&this.db.deleteByIndex(c,{source:d},function(){e.db.insertArray(c,f,null)})};fjs.fdp.SyncManager.prototype.keepSyncProcessItems=function(a,c,d){for(var e=[],f=[],g=[],h=this,j=0;j<a.length;j++){var i=a[j],k=i.xef001type||b.eventTypes.ENTRY_CHANGE,l=i.xpid=i.xpid?i.xpid:d?d+"_"+i.xef001id:i.xef001id;delete i.xef001type;i.source=d;i={eventType:k,feed:c,xpid:l,entry:i};if(this.db)switch(k){case b.eventTypes.ENTRY_CHANGE:e.push(i.entry);g.push(i.entry.xpid);break;
case b.eventTypes.ENTRY_KEEP:f.push(i.entry.xpid);break;default:console.error("Incorrect item change type: "+k+" for Keep sync")}this.fireEvent(c,i)}this.db&&(0<e.length&&(!fjs.fdp.TabsSynchronizer.useLocalStorageSyncronization()||(new fjs.fdp.TabsSynchronizer).isMaster))&&this.db.selectByIndex(c,{source:d},function(a){0>f.indexOf(a.xpid)&&0>g.indexOf(a.xpid)&&h.db.deleteByKey(c,a.xpid,null)},function(){h.db.insertArray(c,e,null)})};fjs.fdp.SyncManager.prototype.processFeedData=function(a,c){this.fireEvent(a,
...
a,c);break;case b.syncTypes.FULL:this.fullSyncProcessItems(f,a,c);break;case b.syncTypes.KEEP:this.keepSyncProcessItems(f,a,c);break;default:console.error("Unknown sync type: "+d)}this.fireEvent(a,{eventType:b.eventTypes.SOURCE_COMPLETE,feed:a,sourceId:c,syncType:d});this.saveVersions(a,c,e)};fjs.fdp.SyncManager.prototype.onSync=function(a){a=fjs.utils.JSON.parse(a);this.syncs.push(a);if(1==this.syncs.length){this.fireEvent(null,{eventType:b.eventTypes.SYNC_START});for(a=0;a<this.syncs.length;a++)for(var c in this.syncs[a])this.syncs[a].hasOwnProperty(c)&&
this.processFeedData(c,this.syncs[a][c]);this.fireEvent(null,{eventType:b.eventTypes.SYNC_COMPLETE});this.syncs=[]}else console.log("!!!!Double onSync!!",this.syncs[0],a)};fjs.fdp.SyncManager.prototype.sendAction=function(a,b,d){d.action=b;this.transport.send({type:"action",data:{feedName:a,actionName:b,parameters:d}})};fjs.fdp.SyncManager.prototype.onClientSync=function(a,b){fjs.fdp.TabsSynchronizer.useLocalStorageSyncronization()&&(new fjs.fdp.TabsSynchronizer).isMaster?fjs.fdp.transport.LocalStorageTransport.masterSend("message",
{type:"sync",data:a}):fjs.fdp.TabsSynchronizer.useLocalStorageSyncronization()&&!b&&fjs.fdp.transport.LocalStorageTransport.masterSend("clientSync",a);if(!fjs.fdp.TabsSynchronizer.useLocalStorageSyncronization()||(new fjs.fdp.TabsSynchronizer).isMaster)this.onSync(a)};fjs.fdp.SyncManager.prototype.getAuthTicket=function(){var a=this;this.state==b.states.READY&&this.db?this.db.clear(function(){a.fireEvent("authError",{type:"authError",message:"Auth ticket wrong or expired"})}):a.fireEvent("authError",
{type:"authError",message:"Auth ticket wrong or expired"})};fjs.fdp.SyncManager.prototype.addFeedListener=function(a,c,d){var e=this;0>this.feeds.indexOf(a)&&this.feeds.push(a);this.superClass.addEventListener.apply(this,arguments);this.status!=b.states.READY?d?0>this.suspendClientFeeds.indexOf(a)&&this.suspendClientFeeds.push(a):(0>this.suspendFeeds.indexOf(a)&&this.suspendFeeds.push(a),0>this.syncFeeds.indexOf(a)&&this.syncFeeds.push(a)):0>this.syncFeeds.indexOf(a)?(d||(this.syncFeeds.push(a),0>
...
function(a,b,d,e,f){0==e.length?f&&f(!1):(this.historyversions[a][b]=e,b={"s.limit":d,"sh.filter":b,"sh.versions":e.join("@")},this.transport.send({type:"loadNext",data:{feedName:a,data:b}}),f&&f(!0))};fjs.fdp.SyncManager.prototype.saveHistoryVersions=function(a,b,d,e){if(this.db&&(!fjs.fdp.TabsSynchronizer.useLocalStorageSyncronization()||(new fjs.fdp.TabsSynchronizer).isMaster))if(this.db.insertOne("historyversions",{feedSourceFilter:a+"_"+b+"_filter",feedName:a,source:b,filter:d,version:e}),b=
this.historyversions[a])delete b[d],b||delete this.historyversions[a]};fjs.fdp.SyncManager.prototype.saveEmptyHistoryVersions=function(a,b){if(this.db&&(!fjs.fdp.TabsSynchronizer.useLocalStorageSyncronization()||(new fjs.fdp.TabsSynchronizer).isMaster))this.db.insertOne("historyversions",{feedName:a,filter:b,version:"-1"}),delete this.historyversions[a]};fjs.fdp.SyncManager.prototype.logout=function(){this.node=this.ticket=null;this.getAuthTicket();this.status=b.states.NOT_INITIALIZED};fjs.fdp.SyncManager.prototype.SFLogin=
function(a){var c=this;this.status==b.states.READY&&this.db?this.db.clear(function(){c.transport.send({type:"SFLogin",data:a})}):this.db?this.suspendLoginData=a:this.transport.send({type:"SFLogin",data:a})}})();namespace("fjs.fdp");
